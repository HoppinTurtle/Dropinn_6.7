<?php
 ...
 error_reporting(0);
	...
	$length		= strlen($baseURL) - strlen('install/index.php');
	
	$length2	= strlen($_SERVER['PHP_SELF']) - strlen('install/index.php');
		...
		trim($_POST['mysql_db']) != '')
	{
		...
		{			
			...
			$sql_file1 = $basePath . '/install1.sql';
			$sql_file2 = $basePath . '/install2.sql';
			$sql_file3 = $basePath . '/install3.sql';
			$sql_file4 = $basePath . '/install4.sql';
			$sql_file5 = $basePath . '/install5.sql';
			$sql_file7 = $basePath . '/install7.sql';
			$sql_file8 = $basePath . '/install8.sql';
$sql_file9 = $basePath . '/install9.sql';
$sql_file10 = $basePath . '/install10.sql';
$sql_file11 = $basePath . '/install11.sql';
	
		        osc_set_time_limit();

			osc_db_install($mysqlDB, $sql_file);
			osc_db_install($mysqlDB, $sql_file1);
			osc_db_install($mysqlDB, $sql_file2);
			osc_db_install($mysqlDB, $sql_file3);
			osc_db_install($mysqlDB, $sql_file4);
			osc_db_install($mysqlDB, $sql_file5);
			osc_db_install($mysqlDB, $sql_file7);
			osc_db_install($mysqlDB, $sql_file8);
osc_db_install($mysqlDB, $sql_file9);
osc_db_install($mysqlDB, $sql_file10);
osc_db_install($mysqlDB, $sql_file11);
 ...
 <?php
	...
	?> 
	                                 <form name="installFrm" method="post" action="">
											  ...
											  <p><input type="text" name="mysql_host" class="clsTextLarge" value=""/><span>*</span></p>
			 ...
			 <p><input type="submit" class="clsBluebtn" onClick="confirm_alert();"  name="submit" value="Submit"  style="cursor:pointer"/>
			 </div>
			 </form>
			 	<?php
    } // End of else

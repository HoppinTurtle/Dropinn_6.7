<!--<link href="<?php echo base_url().'Cloud_data/css_orange/templates/blue/dashboard.css'; ?>" media="screen" rel="stylesheet" type="text/css" />-->
...
<div class="clxfix">
	<div id="dashboard_page" class="container">
<!-- <ul id="nav" class="clearfix col-sm-3 col-md-3 col-xs-12">
	<li id="dashboard" class="col-md-2 col-sm-2 col-xs-12 ">
	<?php
	 if($this->uri->segment(2) == 'dashboard')
	 echo anchor('home/dashboard/',translate("Dashboard"),array("class" => "Search_Active col-md-12 col-xs-12 col-sm-12")); 
		else
		echo anchor('home/dashboard/',translate("Dashboard"),array("class" => "col-md-12 col-xs-12 col-sm-12")); 
		?>
	</li>
	
		<li id="rooms" class="col-md-2 col-sm-2 col-xs-12 ">
	<?php
	 if($this->uri->segment(1) == 'message'  && $this->uri->segment(2) == 'inbox')
	 echo anchor('message/inbox',translate("Inbox"),array("class" => "Search_Active col-md-12 col-xs-12 col-sm-12")); 
		else
		echo anchor('message/inbox',translate("Inbox"),array("class" => "col-md-12 col-xs-12 col-sm-12")); 
		?>
	</li>
	...
	<?php
	 ...
	 echo anchor('listings',translate("Your Listings"),array("class" => "Search_Active col-md-12 col-xs-12 col-sm-12")); 
		else
		echo anchor('listings',translate("Your Listings"),array("class" => "col-md-12 col-xs-12 col-sm-12")); 
		?>
	</li>
	...
	<?php
	 ...
	 echo anchor('travelling/your_trips',translate("Your Trips"),array("class" => "Search_Active col-md-12 col-xs-12 col-sm-12"));
		else
		echo anchor('travelling/your_trips',translate("Your Trips"),array("class" => "col-md-12 col-xs-12 col-sm-12"));
		 ?>
	</li>
 ...
 <li id="rooms" class="col-md-2 col-sm-2 col-xs-12">
	<?php
	 ...
	 echo anchor('account',translate("Account"),array("class" => "Search_Active col-md-12 col-xs-12 col-sm-12"));
		else
		echo anchor('account',translate("Account"),array("class" => "col-md-12 col-xs-12 col-sm-12"));
		?>
	</li>
	
 <li id="rooms" class="clsBg_None col-md-2 col-sm-2 col-xs-12">
	<?php
	 if($this->uri->segment(1) == 'users')
	 echo anchor('users/edit',translate("Profile"),array("class" => "Search_Active col-md-12 col-xs-12 col-sm-12"));
		else
		echo anchor('users/edit',translate("Profile"),array("class" => "col-md-12 col-xs-12 col-sm-12"));
		?>
	</li>
...
<div class="container col-md-12 col-sm-12 col-xs-12" style="margin-bottom:30px;">
<div class="container_bg " id="View_Vouch">

 <div id="dashboard" class="clsDes_Top_Spac">

    <div style="clear:both"></div>
    <div id="left" class="col-md-3 col-sm-3 col-xs-12">
      <div id="user_box">
          <div class="Box_Content_1">
            <div id="user_pic" onClick="show_ajax_image_box();" style="text-align:center; padding:0 0 10px 0;"> 
												
			<img style="height: 200px;"  src="<?php echo $this->Gallery->profilepic($user->id, 2); ?>" />
           </div>
          </div>
          <!-- middle -->
      </div>
    ...
    $this->db->where(array('id'=>$this->uri->segment(3)));
	   ...
	   $this->db->select('verification_status');
					$result = $this->db->get();	
					$v_status=$result->row()->verification_status;	
					$this->db->select('superhost');
					$this->db->where('id',$this->uri->segment(3));
					$sh=$this->db->get('users');
					$shstatus=$sh->row()->superhost;
					
if($user->facebook_verify == 'yes' || $user->google_verify == 'yes' || $user->email_verify == 'yes' || $user->phone_verify == 'yes' || $v_status==1 || $shstatus==1)
	{
    ?>
	...
	<a href="#" data-toggle="tooltip" title="ID Verification">	<img class="badge1" src="<?php echo base_url();?>images/dropinn_verify.png" /> </a>
			</span>
			<?php	}
			  if($shstatus==1){ ?>
			  	<span>
			  	...
			  	<a href="#" data-toggle="tooltip" title="Superhost"><img class="badge2"  src="<?php echo base_url();?>images/super.png" /></a></span>
        	<?php }
        	...
        	?>
          <li class="verifications-list-item" ><i class="fa fa-check-circle-o check_1" aria-hidden="true"></i><b><?php echo translate("Facebook");
		   ?><span style="padding:4px 6px 0px 0px;"><img src="<?php echo base_url();?>images/follow-us-facebook-plus.png" /></span></b><p class="list1">Verified</p>
         ...
         <?php 
			...
			{
        	?>
          <li class="verifications-list-item"><i class="fa fa-check-circle-o check_1" aria-hidden="true"></i><b><?php echo translate("Google"); ?><span><img src="<?php echo base_url();?>images/follow-us-google-plus.png" /></span></b><p class="list1">Verified</p></li>
          ...
          <?php
			...
			{
        	?>
          <li class="verifications-list-item"><i class="fa fa-check-circle-o check_1" aria-hidden="true"></i><b><?php echo translate("Email"); ?><span><img src="<?php echo base_url();?>images/follow-us-email-plus.png" /></span></b><p class="list1">Verified</p></li>
          ...
          <?php
			...
			{
        	?>
          <li class="verifications-list-item"><i class="fa fa-check-circle-o check_1" aria-hidden="true"></i><b><?php echo translate("Phone Number"); ?><span></span></b><p class="" style="color: #82888a;">▒▒▒▒▒▒▒▒▒▒ <?php echo $phone_number;?></p></li>
        
          <?php
			}
			?>
			<a class="add_more" href="<?php echo base_url().'users/verify'?>"><?php echo translate("Add more"); ?></a>
        </ul>
      </div>
      <div style="clear:both"></div>
    </div>
    <?php } ?>
      ...
      <div class="Box_Content" style="padding: 10px;height:auto;">
        <ul class="verification_list">
        	
        <li><?php if($profile->describe != '') echo '<strong>Description</strong><br><spna class="inr-dec">'.$profile->describe;?></span> </li>
        	<?php if($profile->work != '')
			{?>
        <li><?php echo '<strong>Work </strong><br><span class="inr-dec">'.$profile->work; echo'</span>';?></li>
        	<?php } ?>
        <li>
        	<?php
        	if($profile->language != '')
			{ echo '<strong>Languages</strong><br>';
				...
				{
					$i++;
					echo '<span class="inr-dec">'.$this->Common_model->getTableData('language',array('id'=>$row))->row()->name.'</span>';
					if($i != count($language))
					{
					echo ', ';	
					}
    ...
    <div id="main" class="col-md-9 col-sm-9 col-xs-12">				 	
      <div class="Box box_1">
        ...
        <h2 class="name_user"> <?php echo "Hey, I'm".' '.ucfirst($user->username).'!'; ?><!--<span class="Box_Head_Right new"  id="show_date_time"></span>--></h2>
        	<p style="margin-top: 14px;padding-bottom: 0;padding-left:0px;word-wrap: break-word;  font-weight: bold; font-size: 15px;"><?php echo $profile->live.' - '; ?>
      ...
      <div class="Box_Head msgbg msgbh_1">
      ...
      <div class="Box">
      <div class="Box_Head msgbg  msgbh_1">
            <h2 class="sec-tit"><?php echo translate("My Listing"); ?></h2>
          </div>
		 ...
		 foreach($lists->result() as $list)
			{
				?>
								...
								<ul class="even col-md-12 col-xs-12 col-sm-12 padding-none" id="room_<?php echo $list->id; ?>">
									
										<li class="place_image col-md-2 col-sm-3 col-xs-4 padd-zero thumb-size21">
											<a class="thumbnail col-md-12 col-sm-12 col-xs-12" href="<?php echo base_url().'rooms/'. $list->id; ?>">
												<?php 
						                		$date_new = strtotime(date("Y-m-d", strtotime("-3 days")));
												
						                		?>
						               <?php if($list->created >= $date_new ){ ?>
						                		<div class="map_number">New</div>
						                		<?php  }  ?>
											<img style="width:100px;height:95px;" title="Test room" src="<?php echo getListImage($list->id); ?>" onerror="this.src='<?php echo base_url().'images/no_image.jpg';  ?>'" alt="Test room">
											<?php 
						                		$date_new = strtotime(date("Y-m-d", strtotime("-3 days")));
												//echo $date_new;
						               ...
						               <?php if($list->created >= $date_new ){ ?>
						                		<div class="map_number">New</div>
						                		<?php  }  ?>
											<span class="col-md-12 col-xs-12"><img  style="width:100px;height:95px;"  title="Test room" src="<?php echo getListImage($list->id); ?>" alt="Test room"></span></a> </li>
										<li class="main col-md-8 col-sm-8 col-xs-8 thumb-size12"><div class="first-line title"><a class="profile" href="<?php echo base_url().'rooms/'. $list->id; ?>"><?php echo $list->title;?></a></div>

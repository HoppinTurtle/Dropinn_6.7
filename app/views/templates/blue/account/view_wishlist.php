...
.wishlists-container.page-container .pull-left {
    padding-right: 0 !important;
}
</style>
<script>
$( document ).ready(function() {
	var c = $(".wishlists-body").children('div').length;
	$("#wishbodycount").html(c);
});   
</script>
<script>
$(document).ready(function()
...
<div class="wishlists-container page-container"><div class="index_view">
  ...
  <div class="top-bar">
    <div class="col-md-6 col-sm-6 col-xs-12 col-middle img-bot">
  ...
  <a rel="nofollow" title="<?php echo get_user_by_id($user_id)->username;?>" class="pull-left media-photo media-round" href="<?php echo base_url().'users/profile/'.$user_id; ?>">
       ...
       <img width="55" height="55" alt="<?php echo get_user_by_id($user_id)->username;?>" src="<?php echo $this->Gallery->profilepic($user_id, 1); ?>">
  ...
  <div class="media-body">
    <div class="wishlist-header-text">
      <div class="col-md-12 col-sm-12 col-xs-12 col-middle">
        ...
        <div>
          ...
          <strong id="wishbodycount"></strong>
    ...
    <div class="col-middle text-right col-md-6 col-sm-6 col-xs-12">
      <div class="btn-group social-share-widget-container hide"></div>
      <?php
      if($this->uri->segment(2) != 'users_wishlist')
	  {
      ?>
      <div class="col-md-6 col-sm-8 col-xs-12 padding-zero cre-wish">
  ...
  <div class="wishlists-body" id="wishbody">
  ...
  <a class="panel-image media-photo media-link media-photo-block wishlist-unit" href="<?php echo base_url().'account/wishlists/'.$row->id;?>" style="text-decoration:none !important;">
		...
		if($user_wishlist->num_rows() != 0)
			{
			   ...
			   if($data['images']->num_rows() != 0)
			   {
				...
				<div style="background-image:url('<?php echo $image; ?>'),url('<?php echo $noimage; ?>');" class="media-cover media-cover-dark wishlist-bg-img">
   				 </div>
				<?php
			}
			else
				{	$noimage ="'".base_url()."images/no_image.jpg"."'";
					...
					<!-- <div style="background-image:url('<?php echo base_url().'images/'; ?>');" class="media-cover media-cover-dark wishlist-bg-img"> -->
						<div style="background-image:url('<?php echo base_url().'images/'; ?>'),url(<?php echo $noimage; ?>);" class="media-cover media-cover-dark wishlist-bg-img">
   				 </div>
					<?php
				}
  			?>
    ...
    <div class="row-full-height">
      <div class="col-md-12 col-sm-12 col-xs-12 col-middle text-center text-contrast">
        ...
        <?php 
        	...
        	$wishlists_data = $this->Common_model->getTableData('wishlists',array('id'=>$row->id));
	
				if($this->dx_auth->get_user_id() == $wishlists_data->row()->user_id)
				{
				$data['check_user'] = 1;
				$query = $this->db->where('user_wishlist.wishlist_id',$row->id)->join('wishlists','wishlists.id = user_wishlist.wishlist_id')->join('list','list.id = user_wishlist.list_id')->join('property_type','property_type.id = list.property_id')->get('user_wishlist');
				}
				else
				{
				$data['check_user'] = 0;	
				$query = $this->db->where('user_wishlist.wishlist_id',$row->id)->where('wishlists.privacy',0)->join('wishlists','wishlists.id = user_wishlist.wishlist_id')->join('list','list.id = user_wishlist.list_id')->join('property_type','property_type.id = list.property_id')->get('user_wishlist');
				}
        ...
        <div class="btn btn-guest"><?php echo $data['wishlists']->num_rows();?> Listing<?php echo $count_status;?></div>
      </div>
    </div>
  </a>
</div>
...
<div class="modal_save_to_wishlist" style="display: none;">
  ...
  <div class="panel-body" style="text-align: left;">
   <input type="hidden" value="10346170" name="user_id">
    ...
    <input type="text" id="wishlist_name" name="name" class="col-md-11 col-sm-11 col-xs-10">
    ...
    <div>
      <div class="col-md-11 col-sm-11 col-xs-10 col-middle padding-zero">
      ...
      <div class="col-md-1 col-sm-1 col-xs-2 col-middle tool-padd" style="font-size: 14px;">

...
<!--<link href="<?php echo base_url().'css_add/dashboard.css'; ?>" media="screen" rel="stylesheet" type="text/css" />-->
...
$refer=$this->db->query("select * from `referral_management` where `id`=1 ")->row();
		...
		$ref_total=get_currency_value_lys1($refcur,'USD',$refamt);
		if($type==1){
			$trip_amt0=$trip_amt;
			$rent_amt0=$rent_amt;
		$trip=get_currency_value_lys1($refcur,'USD',$trip_amt);
		$rent=get_currency_value_lys1($refcur,'USD',$trip_amt);
		}
		if($type==0){
		...
		if($current==''){
			$current_code=$this->Common_model->getTableData('currency', array('default'=>1))->row()->currency_code;
		}
		else{
...
<div id="dashboard_container" class="clearfix">
  <div id="dashboard_left" class="col-md-3 col-sm-3 col-xs-12 padding-zero" style="padding-top: 0px;">
...
if($verification->facebook_verify == 'yes' || $verification->google_verify == 'yes' || $verification->email_verify == 'yes' || $verification->phone_verify == 'yes')
  ...
  <!-- /left -->
  <div id="dashboard_main" class="col-md-9 col-sm-9 col-xs-12">
...
?>
	...
	$topay = '';
            ...
            <li class="clearfix" <?php if($row->is_read == 0) echo 'style="background:#ececec;border-bottom:1px solid #dbdbdb"'; ?>>
              <div class="clsMsg_User clsFloatLeft col-md-3 col-sm-2 col-xs-4"  style="text-align: center;"> <a href="<?php echo site_url('users/profile').'/'.$row->userby; ?>"><img height="50" class="img-circle" width="50" alt="" src="<?php echo $this->Gallery->profilepic($row->userby,2); ?>" /></a>
                <p><a href="<?php echo site_url('users/profile').'/'.$row->userby; ?>"><?php echo get_user_by_id($row->userby)->username; ?></a> <br />
                  <!--31 minutes-->
                </p>
              </div>
              <div class="clsMeg_Detail clsFloatLeft col-md-6 col-sm-5 col-xs-8 padding-zero padding-zero_2">
              ...
              <div class="clsMeg_Off clsFloatRight  col-md-3 col-sm-3 colxs-12 padding-zero padding-zero_2">
                <p> <span><?php 
	...
	if($check_refer_code->num_rows() == 0)
	{
		...
		{
    ?>
		<div class="Box_dash col-md-12">
				<h2><?php echo translate("Invite your friends, earn");?> <?php echo get_currency_symbol1().get_currency_value($refamt);?>  <?php echo translate("travel credit!"); ?>
...
<?php }
 ...
 <img src="<?php echo base_url();?>images/twshare.png" />
  ...
  $total_refering=$this->db->query("select `ref_total` from `users` where `refer_userid`= '".$this->dx_auth->get_user_id()."'");
            ...
            <div class="stat_box" id="invite_box">
              <a href="<?php echo base_url().'referrals';?>" id="invite_more" class="btn green large btngreen">
               <?php echo translate('Invite More');?>
              </a>
            </div>
          </div>
...
function fb_share()
      	...
      	FB.ui(
  {
    ...
    description: 'Discover and book unique spaces around the world with <?php echo $this->dx_auth->get_site_title();?>. Join now and save $<?php echo $trip;?> off your first trip of <?php echo $rent; ?> or more!'
  },
  function(response) {

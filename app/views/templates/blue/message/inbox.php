...
$this->load->helper('text');
	...
	$topay = '';
			 ...
			 $message_id = $this->db->where('userby',$row->userby)->where('userto',$row->userto)->order_by('conversation_id','desc')->get('messages')->row()->conversation_id;
																			...
																			?>	
<li class="clearfix col-md-12" <?php if($row->is_read == 0) echo 'style="background:#ececec;  color:#00B0FF"'; else echo 'style="background:#FFFFD0; background-color: white; color:#00B0FF"';  ?> >
                    	<div class="clsMsg_User clsFloatLeft col-md-2 col-sm-2 col-xs-12">
                        	<a class="col-md-6 col-sm-12 padding-zero" href="<?php echo site_url('users/profile').'/'.$row->userby; ?>"><img height="50" width="50" alt="" src="<?php echo $this->Gallery->profilepic($row->userby,2); ?>" /></a>
                            <p class="col-md-6 col-xs-6 col-sm-9"><a href="<?php echo site_url('users/profile').'/'.$row->userby; ?>"><?php echo get_user_by_id($row->userby)->username; ?></a> <br />
                            <!--31 minutes-</p>
                        </div>
                        <div class="clsMeg_Detail clsFloatLeft col-md-5 col-sm-6 col-xs-12">
...
$subject = 'Inquiry about '.substr(get_list_by_id($row->list_id)->title,0,17);
              ...
              <div class="clsMeg_Off clsFloatRight col-md-2 col-xs-6 col-sm-2">
                <p> <span><?php 				
              ...
              <?php } ?>
							...
							<div class="clsMsg_Del clsFloatLeft col-md-3 col-sm-2 col-xs-6 ">
...
$subject = 'Inquiry about '.substr(get_list_by_id($row->list_id)->title,0,17);
														...
														{ 
															...
															else if($row->message_type == 1 || $row->message_type == 12) {      
              ...
              <div class="clsMeg_Off clsFloatRight col-md-2 col-xs-6 col-sm-2">
	           ...
	           echo 'List Deletion';
					...
					if($row->message_type==7 || $row->message_type==8 || $row->message_type==1 || $row->message_type== 12) 
					{
					if($row->conversation_id!=0)
					{
              ...
              <?php } ?>
																								...
																								<div class="clsMsg_Del clsFloatLeft col-md-3 col-sm-2 col-xs-6 padding-zero_2">
																								<?php if($row->is_starred == 0) $class = "clsMsgDel_Unfil"; else $class = "clsMsgDel_fil"; ?>
                    	     <p class="clearfix">
																										<span><a class="<?php echo $class; ?>" id="starred_<?php echo $row->id; ?>" href="javascript:void(0);" onclick="javascript:starred('<?php echo $row->id; ?>');"></a></span>
																										<span><a onclick="javascript:deleted('<?php echo $row->id; ?>');" href="javascript:void(0);" id="delete_<?php echo $row->id; ?>"><?php echo translate("Delete"); ?></a></span>

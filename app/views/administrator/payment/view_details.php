...
<table class="table" cellpadding="2" cellspacing="0">
 <tr>
  ...
  <td><a target="_blank" href="<?php echo base_url().'rooms/'.$result->list_id;?>" ><?php echo get_list_by_id($result->list_id)->title; ?></a>
  	</td>
 </tr>
 ...
 <tr>
  ...
  <td>
		<a target="_blank" href="<?php echo base_url().'users/profile/'.$result->userto;?>" ><?php echo $hotelier_name; ?></a>
  ...
  <td>
  			<a target="_blank" href="<?php echo base_url().'users/profile/'.$result->userby;?>" ><?php echo $booker_name; ?></a>
 ...
 <tr>
  <td class="clsName"><?php echo translate_admin('No Of Guest'); ?></td>
  <td><?php echo $result->no_quest; ?></td>
 </tr>
 ...
 <tr>
  ...
  <td><?php echo $this->Common_model->getTableData('booktime_policy',array('reservation_id'=>$result->id))->row()->name; ?></td>
 </tr>

		<tr>
  <td class="clsName"><?php echo translate_admin('Total nights'); ?></td>
  <td><?php 
	$diff              = $result->checkout - $result->checkin;
	$days = ceil($diff/(3600*24));
 	echo $days ;
  ?></td>
 </tr>
 		<tr>
  <td class="clsName"><?php echo translate_admin('Per night price'); ?></td>
  <td><?php 
 	 echo $currency_code.' '.$currency_symbol.$result->per_night_price; 
  ?></td>
 </tr>
 		<tr>
  <td class="clsName"><?php echo translate_admin('Cleaning Fee'); ?></td>
  <td><?php 
 	 echo $currency_code.' '.$currency_symbol.$result->leaning; 
  ?></td>
 </tr>
 		<tr>
  <td class="clsName"><?php echo translate_admin('Security Fee'); ?></td>
  <td><?php 
 	 echo $currency_code.' '.$currency_symbol.$result->security; 
  ?></td>
 </tr>
 		<tr>
  <td class="clsName"><?php echo translate_admin('Additional guest Fee'); ?></td>
  <td><?php 
 	 echo $currency_code.' '.$currency_symbol.$result->extra_guest_price; 
  ?></td>
 </tr>
 
  		<tr>
  <td class="clsName"><?php echo translate_admin('Subtotal'); ?></td>
  <td><?php 
 	 echo $currency_code.' '.$currency_symbol.$result->topay; 
  ?></td>
 </tr>
		<tr>
...
else
{
 ...
 <?php } if(isset($result->cleaning))
 {?>
 <tr>
  <td class="clsName"><?php echo translate_admin('Cleaning fee'); ?></td>
  <td><?php echo $currency_code.' '.$currency_symbol.$result->cleaning; ?></td>
 </tr>
 ...
 <tr>
  <td class="clsName"><?php echo translate_admin('Security fee'); ?></td>
  <td><?php echo $currency_code.' '.$currency_symbol.$result->security; ?></td>
 </tr>
<?php
			...
			if($result->is_payed != 1)
				{
					if($result->status == 2 || $result->status == 4 || $result->status == 5 || $result->status == 6 || $result->status == 11 || $result->status == 12 || $result->status == 13)
					{
						if(!isset($guest_topay) && !isset($host_topay))
						{
							if($result->transaction_id == "0" && $result->is_payed_guest != 1)
							{
							?>
							<input type="hidden" name="biz_id" value="<?php echo $guest_payout_id->email; ?>" />
							<input type="hidden" name="to_guest" value="1" />
    					   <input style="margin:0px 0px 0px 0px !important;" class="clsSubmitBt1" type="submit" name="payviapaypal" value="<?php echo translate_admin('Pay Using PayPal to Traveller'); ?>" style="width:250px;" />
							...
							<?php
							 ...
							 else if(isset($guest_payout_id->email) && $guest_payout_id->recipient_id !="" && $result->is_payed_guest != 1)
							 {
								
							 ?>
							 <input type="hidden" name="wallet_id" value="<?php echo $guest_payout_id->recipient_id; ?>" />
							 <input type="hidden" name="bank_id" value="<?php echo $guest_payout_id->bank_accountid; ?>" />
							 <input type="hidden" name="topaytype" value="guest" />
    						 <input class="clsSubmitBt1" type="submit" name="payviastripe" value="<?php echo translate_admin('Pay Using stripe to Traveller'); ?>" style="width:250px;" />
							 <?php
							}
					...
					<?php
							 ...
							 else if(isset($guest_payout_id->email) && $result->payment_id == "4" && $result->is_payed_guest != 1)
						  {
							  if($guest_topay != 0)
							 {
						 ?>
						  <input type="hidden" name="guest_biz_id" value="<?php echo $guest_payout_id->recipient_id; ?>" />
					  <input type="hidden" name="bank_id" value="<?php echo $guest_payout_id->bank_accountid; ?>" />
    					 <input class="clsSubmitBt1" type="submit" name="payviastripe_guest" value="<?php echo translate_admin('Pay Using stripe to Traveller'); ?>" style="width:250px;" />
					  <?php
						 	}
						}
						else if($result->is_payed_guest == 1)
					...
					else if($result->status == 7 || $result->status == 10 || $result->status == 8 || $result->status == 9)
					{
						...
						<?php 
											 ...
											 else if(isset($host_payout_id->email) && $host_payout_id->recipient_id !="" )
	 {
		 ?>
	 <input type="hidden" name="biz_id" value="<?php echo $host_payout_id->email; ?>" />
						 <input type="hidden" name="wallet_id" value="<?php echo $host_payout_id->recipient_id; ?>" />
    					 <!--<input class="clsSubmitBt1" type="submit" name="payviapaypal" value="<?php echo translate_admin('dont use'); ?>" style="width:190px;" />-->
    					  <input type="hidden" name="topaytype" value="host" />
    					 	<input type="hidden" name="bank_id" value="<?php echo $host_payout_id->bank_accountid; ?>" />
    					 <input class="clsSubmitBt1" type="submit" name="payviastripe" value="<?php echo translate_admin('Pay Using stripe to Host'); ?>" style="width:215px;" />
					 <?php
	}
						...
						?>
						<input type="hidden" name="biz_id" value="<?php echo $host_payout_id->email; ?>" />
						<input type="hidden" name="to_host" value="1" />
 ...
 <tr><td></td>
	...
	<?php
		 ...
		 else if($check_accept_pay->row()->transaction_id != ''){
	 if(isset($host_payout_id->email) && $check_accept_pay->row()->payout_type == 4 ){ ?>
	 <input type="hidden" name="biz_host_id" value="<?php echo $host_payout_id->recipient_id; ?>" />
	 <input type="hidden" name="bank_id" value="<?php echo $host_payout_id->bank_accountid; ?>" />
	  <input type="hidden" name="topaytype" value="host" />
	 <input class="clsSubmitBt1" type="submit" name="payviastripe_accept_pay" value="<?php echo translate_admin('Pay Using stripe to Host'); ?>" style="width:210px;" />
	 <?php } }

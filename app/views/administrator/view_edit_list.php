...
margin-bottom: -30px;
background: url(images/single_cal_btt_arrow.png) no-repeat bottom center;
...
jQuery(document).ready(function () {
    ...
    google.maps.event.addListener(autocomplete2, 'place_changed', function() {
      ...
      address = data.results[0].formatted_address;
 	   	jQuery('#hidden_address_edit').val(addr);
 	   	 var state_status = 0;
         	   	var addr = {};
   ...
   document.getElementById(component).value = '';
    document.getElementById(component).disabled = false;
    	...
    	document.getElementById(addressType).value = val;
    		...
    		//$("#str_zipcode2").text(val);
    		if(jQuery("#postal_code").val()==""){
    		jQuery("#postal_code").val(jQuery("#postal_code_prefix").val());	
    		}
     ...
     for (var s1 = 0; s1 < data.results.length; s1++) {
    for (var s = 0; s < data.results[s1].address_components.length; s++) {
                	var te = data.results[s1].address_components[s].types[0];
                    var o = data.results[s1].address_components[s].types;
                    
                    if (o == "neighborhood,political") {
                        var neigh = data.results[s1].address_components[s].long_name ;
                        
                        document.getElementById("neighborhood").value = neigh;
                        
                    }
                    if(o == "sublocality_level_1,sublocality,political"){
                        var subp = data.results[s1].address_components[s].long_name ;
                        
                          document.getElementById("district").value = subp;
                    }
                   }
                  }
    ...
    dataType: 'json',
       ...
       jQuery.each(data, function(key, value)
			  {
			  	city = value['city'];
			  	jQuery('#locality').val(city);
			  	state = value['state'];
			  	 jQuery('#administrative_area_level_1').val(state);
                country = value['country'];
                jQuery('#country').val(country);
                jQuery('#hidden_lat_edit').val(value['lat']);
                jQuery('#hidden_lng_edit').val(value['long']); 
                
                jQuery('#postal_code').val(value['zip_code']);
			  })
   	}
	})
	  }
...
function mpdrg() {
 	   ...
 	   jQuery.getJSON("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + lat + "," + lng + "&sensor=true&key="+places_API+"&language=en", function( data ) {
     ...
     jQuery('#hidden_lng_edit').val(lng);
 	   	...
 	   	jQuery("#lys_street_address_edit").val(address);
 	   	var addr = {};
      ...
      jQuery("#str_street_address2_edit").text(data.results[0].formatted_address);
  		// Get each component of the address from the place details
  		// and fill the corresponding field on the form.
  		for (var component in componentForm) {
    document.getElementById(component).value = '';
    document.getElementById(component).disabled = false;
    	...
    	document.getElementById(addressType).value = val;
    	if(addressType=="postal_code" || addressType=="postal_code_prefix" ){
    		jQuery("#str_zipcode2_edit").text(val);
    		if(jQuery("#postal_code").val()==""){
    		jQuery("#postal_code").val(jQuery("#postal_code_prefix").val());	
    		}
    	}
    	if(addressType == "locality"){
    		jQuery("#str_city_state_address2_edit").text(val) ;
    	}
    		}
  		}
  		for (var s1 = 0; s1 < data.results.length; s1++) {
...
<div id="View_Edit_List">
	...
	<div class="">
	 ...
	 {?>	
            ...
            <span4><input type="submit" id="aminitiesA" onclick="javascript:showhide('2');" value="<?php echo translate_admin('Amenities'); ?>"></span4>
...
<div class="ui-widget" style="float:left; margin-left: 78px;">
<label>
Change Host:
<select style="width:200px;" class="fixed-width" id="user_det" name="user_det">
	
	<option selected disabled value=""><?php echo translate("Select...");?>
	<?php 
	$get_users = get_all_Users();
	
		foreach($get_users->result() as $row_users)
		{ ?>
			<option value="<?php echo $row_users->id;?>" <?php if($row_users->id == $result->user_id) echo 'selected=selected'; ?>> <?php echo ucfirst($row_users->username)." (".$row_users->id.")"; ?></option>
	<?php	} ?>
			
</select>
</label>

<button class="btn" id="ch_host">Save</button>
</div>
...
<?php  
					...
					foreach($room->result() as $value) {
						  ?> <option value="<?php echo $value->id;?>" <?php if($result->room_id == $value->id) echo 'selected=selected'; ?>> <?php echo $value->type; ?></option>
					<?php }?>
</select>
<lable class='error' id="hosting_room_type_error" style="display: none;">Please choose the Room Type.</label>
</td>
...
<p class="clearfix">
				...
				$i = 1; $j = 1; 
				foreach($amnities->result() as $rows) { if($i == 1) echo '<ul class="amenity_column col-xs-12 col-md-offset-4 col-sm-offset-4">'; ?>
							<li>
									...
									<label for="amenity_<?php echo $j; ?>"><?php echo $rows->name; ?> <a title="<?php echo $rows->description; ?>" id="tip"><i class="fa fa-caret-down drop_img" aria-hidden="true"></i></a> </label>
							</li>
...
<div class="col-xs-12 col-md-offset-4 col-sm-offset-4">
<p><input type="submit" name="update_aminities" value="<?php echo translate_admin("Update"); ?>"/></p>
...
<p style="text-align:left; margin: 10px 0px 10px 0px;">
 ...
 <?php 
  echo '<div id="galleria_container">'; 
		...
		{
			echo '<ul class="clearfix" style="padding: 0px;">';
			$i = 1;
			...
			{		
			  ...
			  $url = base_url().'images/'.$image->list_id.'/'.$image->name;
			   ...
			   echo '<p><label><input type="checkbox" name="image[]" value="'.$image->id.'" /></label>';
  				echo '<img src="'.$url.'" width="150" height="150" style="margin: 0px 9px;"/><input type="radio" '.$checked.' name="is_main" value="'.$image->id.'" /></p>';
						echo '<div class="panel-body panel-condensed">
      							<textarea class="highlights" rows="3" cols="18" class="highlight" name="highlight" id="highlight_'.$image->id.'" placeholder="'.translate_admin("What are the highlights of this photo?").'" class="input-large" onkeyup="highlight1('.$image->id.')">'.trim($image->highlights).'</textarea>
      								</div></li>';
						$i++;
			}
			echo '</ul>';
...
<table class="table photo" cellpadding="2" cellspacing="0" style="width: 100%;">
<tr>
<input type="hidden" name="list_id" value="<?php echo $result->id;  ?>">
<td style="padding-left:0px;"><span style="margin:0 10px 0 0;"> <?php echo translate_admin("Upload new photo"); ?> </span></td>
<tr><input id="new_photo_image" name="userfile[]"  size="24" type="file" multiple="true"/></tr>
</tr>
<tr>
	<!-- <td>&#160;</td> -->
...
<td>    
<select style="margin:0px 0px 0px 0px;" id="hosting_guests_included" name="guests">
<?php for($i = 1; $i <= 16; $i++) { ?>
...
?>
<div id="calendar">
<div class="col-md-12 col-xs-12 col-sm-12 table_Line">
<?php
}
else {
	?>
...
<input type="hidden" name="hidden_lng" id="hidden_lng_edit"/>
  <tr>
    ...
    <td style="padding: 20px 0px 0px 0px;">
    <input type="text" size=34 placeholder="e.g. Apt #7" class="input-large" value="<?php echo $optional_address; ?>" id="street_number" name="apt">
    </td>
  </tr>
<tr>
  	<td style="padding: 20px 0px 0px 0px;">
    <label for="city" class="label-large"><?php echo translate_admin("City");?></label></td><td style="padding: 20px 0px 0px 0px;">
    <input type="text" size=34 placeholder="e.g. San Francisco" class="input-large" value="<?php echo $city; ?>" id="locality" name="city">
</td>
  </tr>
<tr>
  	<td style="padding: 20px 0px 0px 0px;">
    <label for="state" class="label-large"><?php echo translate_admin("State");?></label></td><td style="padding: 20px 0px 0px 0px;">
    <input type="text" size=34 placeholder="e.g. CA" class="input-large" value="<?php echo $state; ?>" id="administrative_area_level_1" name="state">
    </td>
     </tr>
<tr>
  	<td style="padding: 20px 0px 0px 0px;">
    <label for="zipcode" class="label-large"><?php echo translate_admin("ZIP Code");?></label></td><td style="padding: 20px 0px 0px 0px;">
    <input type="text" size=34 placeholder="e.g. 94103" class="input-large" value="<?php echo $zip_code; ?>" id="postal_code" name="zipcode" onkeypress="return (event.which >= 48 && event.which <= 57)     || event.which == 8 || event.which == 46">
...
<strong id="str_postal_code"></strong><br>
...
<strong id="str_street_address2_edit"></strong><br>
<strong id="str_city_state_address2_edit"></strong><br>
<strong id="str_zipcode2_edit"></strong><br>
<strong id="str_country2_edit"></strong><br>
...
<div style="border:none;" class="panel-footer">
   <!-- <button class="cancel-btn" id="cancel_popup3">
    ...
    <!-- <input type="submit"  class="pin-on-map enable_finish" id="finish_popup3" style="display: none" value="<?php echo translate("Finish");?>"> -->
    <input type="submit" id="finish_popup3" class="pin-on-map disable_finish"  style="opacity: 0.65;" value="<?php echo translate("Finish");?>" > 
...
<link href="<?php echo base_url(); ?>css/templates/blue/calendar_single.css" media="screen" rel="stylesheet" type="text/css" />
...
function centerLightbox() {
   ...
   var g_mouse_y = jQuery('#mouse_y').val();
    $('lwlb_calendar2').setStyle({ position:"absolute", left: (g_mouse_x - (boxWidth / 0.79))+"px", top: (g_mouse_y - boxHeight - 155)+"px"});
}
                    ...
                    <div class="inner">
                      <form action="<?php echo site_url('calendar/modify_calendar').'?month='.$month.'&year='.$year; ?>" method="post" onSubmit="before_submit();; new Ajax.Request('<?php echo site_url('calendar/modify_calendar').'?month='.$month.'&year='.$year; ?>', {asynchronous:true, evalScripts:true, parameters:Form.serialize(this), onSuccess: function(transport){ window.location.href = '<?php echo base_url();?>administrator/lists/managelist/<?php echo $result->id;?>?month=1'; }}); return false;">
                        ...
                        <div id="lwlb_normal_section">
                          <div class="header bottom_line">
                            <div id="lwlb_hosting_name" class="header_text" style="float:left; width:280px;border-bottom: 1px solid #c3c3c3;text-align: center; padding: 5px;">hosting name</div>
                            <div class="close"><a href="#" onClick="lwlb_hide_special();return false;"><img src="<?php echo css_url(); ?>/images/sin_cal_close.gif" /></a></div>
                            <div class="clear"></div>
                          </div>
			...
			jQuery(document).ready(function()
{
   ...
   jQuery("#ch_host").on('click',function(){
   	//alert();return false;
   	var change_user = jQuery("#user_det").val();
   	var ok = confirm("Are you sure you want to change the user?");
   		if(!ok)
		{
			return false;
		}
   	 ...
   	 jQuery.ajax({
      url: '<?php echo base_url()."administrator/lists/change_user/".$result->id."/";?>'+change_user,
      success: function(data) {
      	if(jQuery.trim(data)=="fail"){
      		alert("This has at least one contact requests/reservation!");
      	}
 ...
 jQuery('#edit_address1').click(function()
     {
     	...
     	if(jQuery.trim(jQuery('#lys_street_address_edit').val()) == '' && jQuery.trim(jQuery('#locality').val()) == '' && jQuery.trim(jQuery('#postal_code').val()) == '')
     	{
  ...
  url: '<?php echo base_url()."rooms/add_address";?>',
   data: { room_id: <?php echo $result->id;?>, country: jQuery('#country').val(), city: jQuery('#locality').val(), state: jQuery('#administrative_area_level_1').val(), street_address: jQuery('#lys_street_address_edit').val(), optional_address: jQuery('#street_number').val(), zipcode: jQuery('#postal_code').val(), lat: jQuery('#hidden_lat_edit').val(), lng: jQuery('#hidden_lng_edit').val(), full_address: jQuery('#hidden_address_edit').val() },
   success: function(data)
        {
      jQuery('#str_street_address').replaceWith('<strong id="str_street_address">'+jQuery('#lys_street_address_edit').val()+'</strong>');
       jQuery('#str_city_state_address').replaceWith('<strong id="str_city_state_address">'+jQuery('#locality').val()+'  '+jQuery('#administrative_area_level_1').val()+'</strong>');
       jQuery('#str_country').replaceWith('<strong id="str_country">'+jQuery('#country').val()+'</strong>');
       jQuery('#str_postal_code').replaceWith('<strong id="str_postal_code">'+jQuery('#postal_code').val()+'</strong>');
       jQuery('#str_street_address1_edit').replaceWith('<strong id="str_street_address1_edit">'+jQuery('#lys_street_address_edit').val()+'</strong>');
       jQuery('#str_city_state_address1_edit').replaceWith('<strong id="str_city_state_address1_edit">'+jQuery('#locality').val()+'  '+jQuery('#administrative_area_level_1').val()+'</strong>');
       jQuery('#str_country1_edit').replaceWith('<strong id="str_country1_edit">'+jQuery('#country').val()+'</strong>');
       jQuery('#str_zipcode1_edit').replaceWith('<strong id="str_zipcode1_edit">'+jQuery('#postal_code').val()+'</strong>');
       jQuery('#str_street_address2_edit').replaceWith('<strong id="str_street_address2_edit">'+jQuery('#lys_street_address_edit').val()+'</strong>');
       jQuery('#str_city_state_address2_edit').replaceWith('<strong id="str_city_state_address2_edit">'+jQuery('#locality').val()+'  '+jQuery('#administrative_area_level_1').val()+'</strong>');
       jQuery('#str_country2_edit').replaceWith('<strong id="str_country2_edit">'+jQuery('#country').val()+'</strong>');
       jQuery('#str_zipcode2_edit').replaceWith('<strong id="str_zipcode2_edit">'+jQuery('#postal_code').val()+'</strong>');	
...
<style>
	...
	#calendar2 .tile, #calendar2 .tile_selected, #calendar2 .day_header
	{
		width: 14% !important;
	}
	.sync_cal
	...
	#galleria_container .panel-body.panel-condensed{
		padding: 30px 0px;
	}
	textarea.highlights {
min-width: 20px;

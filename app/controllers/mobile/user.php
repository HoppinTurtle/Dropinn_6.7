...
public function index1()
	...
	public function login()
	{
		...
		$onesignal_id1 = $this->input->get('onesignal_id');
                ...
                //print_r("<br>".$stored_hash);exit;
					...
					{	
					  $profile_pic = $this->Gallery->profilepic($row->id, 2);
					   $onesignal_id ['onesignal_id']= $onesignal_id1;
					  $this->db->where('id',$row->id)->update('users',$onesignal_id);
...
public function logout()
{
		...
		if($id==2)
		{
			...
			$onesignal_id['onesignal_id']='nil';
		}
		
		$this->db->where('id',$userid)->update('users',$onesignal_id);
		 echo '[{"status":"Success"}]';
}
...
public function view_profile()
	...
	public function view_sinchprofile()
	{
		$user_id	= $this->input->get("user_id");
		...
		if( strlen($password) < 4)
		{
		  echo '[{"status":"Sorry! Password has too less characters.","success":"0"}]';exit;	
			...
			//$this->db->insert('users',$add1);	
			  $onesignal_id ['onesignal_id']= $onesignal_id1;	
			$this->db->where('id',$data['user_id'])->update('users',$onesignal_id);
...
public function fb_signup()
{
 		...
 		if( !$this->dx_auth->is_email_available($email_id) )
		{
				...
				echo "[{\"status\":\"Sorry! This email has already been registered\",\"user_id\":\"".$users['id']."\",\"first_name\":\"$fname\",\"last_name\":\"$lname\",\"email\":\"$email_id\",\"profile_pic\":\"$src\"}]";exit;				
		}
        else
        {
		  ...
		  if($id_query->num_rows() != 0)
          {
		   ...
		   // $img['src'] = $src;
			...
			$this->db->where('username',$username)->update('users',$update_data);
			  $onesignal_id ['onesignal_id']= $onesignal_id1;
            $this->db->where('id',$id)->update('users',$onesignal_id);
			echo "[{\"status\":\"Successfully registered\",\"user_id\":\"$id\",\"email\":\"$email_id\",\"first_name\":\"$fname\",\"last_name\":\"$lname\",\"profile_pic\":\"$src\"}]";
        }
}
    ...
    public function fb_signin()
    {
        ...
        if($query->num_rows() != 0)
        {
            ...
            else
            {
			  ...
			  $onesignal_id ['onesignal_id']= $onesignal_id1;
			$this->db->where('id',$user_id)->update('users',$onesignal_id);
...
public function google_signup()
{
		...
		$start_date =  strtotime($final_date);
		 ...
		 //$row_email= $id_query1->row()->email;
          ...
          {
			$img['email'] = $email_id;
		    $img['src'] = str_replace("?sz=50", "?sz=250", $src ) ;
			//$this->db->where('email',$email_id)->update('profile_picture',$img);
          }
		  else
		  	{
 		...
 		if( !$this->dx_auth->is_email_available($email_id) )
		{
				...
				$this->load->model('users_model');
				$users=$this->users_model->getUserIdByEmail($email_id);
				echo "[{\"status\":\"Sorry! This email has already been registered\",\"user_id\":\"".$users['id']."\",\"email\":".'"'.$email_id.'"'.",\"first_name\":".'"'.$fname.'"'.",\"last_name\":".'"'.$lname.'"'.",\"profile_pic\":".'"'.$src.'"'."}]";exit;				
		}
        else
        {
           $id_query = $this->db->select('id')->limit(1)->order_by('id','desc')->from('users')->get();
		  ...
		  if($id_query->num_rows() != 0)
          {
		   ...
		   // $img['src'] = $src;
			...
			$update_data['onesignal_id']= $onesignal_id1;
			$this->db->where('email',$email_id)->update('users',$update_data);
...
public function facebook()
	...
	foreach($query->result() as $row){
     ...
     $data['colud_api_secret']    = $this->db->get_where('settings', array('code' => 'cloud_api_secret'))->row()->string_value;
	   $data['sinch_name']    = $this->db->get_where('settings', array('code' => 'sinch_name'))->row()->string_value;
     $data['sinch_api_key']    = $this->db->get_where('settings', array('code' => 'sinch_api_key'))->row()->string_value;
     $data['sinch_api_secret']    = $this->db->get_where('settings', array('code' => 'sinch_api_secret'))->row()->string_value;
	$data['onesignal_api_id']    = $this->db->get_where('settings', array('code' => 'onesignal_api_id'))->row()->string_value;
    $data['onesignal_project_number']    = $this->db->get_where('settings', array('code' => 'onesignal_project_number'))->row()->string_value;
...
public function sendMessage(){
		...
		$content = array(
			"en" => 'You have a new reservation request'
			);
		
		$fields = array(
			'app_id' => "d020a06b-63da-4695-bfe3-d1f62f2b839d",
			'include_player_ids' => array("dee7c12e-184c-4f5c-b336-e88c73fe2745"),
			'data' => array("foo" => "bar"),
			'contents' => $content,
			'android_group' => '2',

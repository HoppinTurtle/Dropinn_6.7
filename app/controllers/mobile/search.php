...
//             
		...
		$empty_image = cdn_url_images().'images/no_avatar.jpg';
		$empty_image1 = cdn_url_images().'images/no_image.jpg';
		if(!empty($image)){
					$data['image']   = $image;
				}
                else{
		   ...
		   if($query_price->num_rows() != 0)
			{
				foreach($query_price->result() as $row_price)
				{
					$room_id = $row_price->id;
					 ...
					 $currencycode= $row_price->currency;
					  $check_price = (get_currency_value_lys($currencycode,$common_currency,1)) * $row_price->price;

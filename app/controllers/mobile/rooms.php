...
public function add1()
	...
	$data['calendar_type'] = 2;
		$data['capacity'] 		= $capacity;
		//$data['imageurl'] 		= $this->input->get('imageurl');
		$data['phone'] 			= $this->input->get("phone");
        $data['list_pay']   = 0;
		$data['is_enable']   = 0;
		$data['max_stay']=$this->input->get('max_stay');
		$data['min_stay']=$this->input->get('min_stay');
		//$data['cancellation_policy'] 	= $this->input->get('cancellation_policy');
		$data['cancellation_policy'] 	= 1;
...
public function your_listing()
{
	...
	foreach($result as $row)
	{
		...
		else 
		{
            ...
            $total_status = $row->overview + $row->price_status + $row->photo_status + $row->address_status + $row->listing_status;
            
            $data['step_count'] = 5 - $total_status;
            
            $data['list_status']=$row->listing_status;
...
public function edit_room()
{
	$this->load->model('common_model');
		...
		$data12['room_type']			 = $this->input->get("room_type");
		$data12['home_type']			 = $this->input->get("home_type");
		
		if(!empty($data12['room_type']))
		{
			$data['room_type']			 = $data12['room_type'];
		}
		
		if(!empty($data12['home_type']))
		{
			$data['home_type']			 = $data12['home_type'];
		}
...
public function view_room()
{
	    ...
	    {
	    	$row = $result->row();
	           echo '[{"status":"Edit Your Details","user_id":"'.$row->user_id.'","room_id":"'.$row->id.'","beds":"'.$row->beds.'","bedrooms":"'.$row->bedrooms.'","amenities":"'.$row->amenities.'","bathrooms":"'.$row->bathrooms.'","guest":"'.$row->capacity.'","room_type":"'.$row->room_type.'","home_type":"'.$row->home_type.'","max_stay":"'.$row->max_stay.'","min_stay":"'.$row->min_stay.'"}]';
			exit;
	     }
	     else {
...
public function view_listing()
{
		...
		$list_status = $this->db->where('id',$roomid)->get('lys_status');
		
		if($list_status->num_rows() !=0)
        {
    foreach ($list_status->result() as $row)
	{
		    $data['title_status'] = $row->title;
			$data['address_status'] = $row->address;
...
/*public function add_room1()
	...
	{
                    $data1['list_id'] = $this->input->get('roomid');
					$datay['title'] 	  = $this->input->get('title');
					$data['desc'] 	  = $this->input->get('description');
					$data['price'] 	  = $this->input->get('price');
					$data['calendar_type'] 	  = 2;
...
public function apartment_detail(){
	...
	foreach($query->result() as $row)
	{
		...
		$data['step_status'] = $row->step_status;
		$data['city']  = $row->city;
		$data['cancellation_policy']  = $row->cancellation_policy;
		$data['max_stay']  = $row->max_stay;
		$data['min_stay']  = $row->min_stay;
		$data['room_type']=$row->room_type;
		$data['bedrooms']=$row->bedrooms;
...
//                 
            ...
            $data['common_currency_value'] = (get_currency_value_lys($currencycode,$common_currency,1)) * $row->price;
                ...
                $data['username'] = $query2->row()->username;
...
public function checkin()
		...
		if($query->num_rows()!=0)
	{
		...
		$onesignal_id               = $query2->row()->onesignal_id;
		...
		$phnumH 											= '';
			...
			$this->sendMessage($onesignal_id, $insertData['message']);
			/*
			$fields = array(
						'app_id' => "d020a06b-63da-4695-bfe3-d1f62f2b839d",
						'include_player_ids' => array($onesignal_id),
						'data' => array("foo" => "bar"),
						'contents' => array(
						"en" => $insertData['message']
						),
						'android_group' => '2',
					);
					...
					curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json; charset=utf-8',
															   'Authorization: Basic MmUzNmQ0NjItMjQ5Ni00Mzk5LWJmM2UtYzUwNzFhZDYyYjI4'));
...
public function host_review()
		...
		if($query->num_rows()!=0)
	{
		...
		$traveler_name 				= $query1->row()->username;
		$traveler_email 			= $query1->row()->email;
		$onesignal_id               = $query1->row()->onesignal_id;
		//$query2     						 = $this->Users_model->get_user_by_id($row->userto);
		$query2 = $this->db->where('id',$row->userto)->get('users');
		$host_name 								= $query2->row()->username;
		$host_email 							= $query2->row()->email;
...
public function cancel()
	...
	if($query->num_rows()!=0)
	{
					...
					// $data['checkin'] = "0";
					// $data['checkout'] = "0";
					$get_reserv = $this->Common_model->getTableData('reservation', array('id' => $reservid))->row();
					
					if($status == 5 || $status == 11)
					{
					$query2 = $this->db->where('id',$get_reserv->userto)->get('users');
					$onesignal_id  = $query2->row()->onesignal_id;
					}
					else if($status == 6 || $status == 12 || $status == 13 )
					{
					$query2 = $this->db->where('id',$get_reserv->userto)->get('users');
					$onesignal_id  = $query2->row()->onesignal_id;	
					}	
					...
					{
					echo '[{"status":"Invalid status id"}]'; exit;
					}	
					$this->sendMessage($onesignal_id, "Sorry, The booking is cancelled");
...
{
	$this->sendMessage("9607737d-dd32-40c0-b854-e72f9256ad55", "adas");
}*/


public function sendMessage($playerid, $message){
	
	 $appid   = $this->Common_model->getTableData('settings', array('code' => 'onesignal_api_id'))->row()->string_value;
     $appkey   = $this->Common_model->getTableData('settings', array('code' => 'onesignal_api_key'))->row()->string_value;

...
class Users extends CI_Controller
{
   ...
   //include_once APPPATH."libraries/linkedin_OAuth/linkedClass.php";
	...
	public function success()
	{
		...
		$data12['last_login'] = time();
		$this->Users_model->set_user($this->dx_auth->get_user_id(), $data12);
	...
	public function view_twitter_popup(){
	...
	//	if( ($this->dx_auth->is_logged_in()) || ($this->facebook_lib->logged_in()) || ($this->twitter->is_logged_in()) )
		...
		if($this->input->post())
		{
		  $this->form_validation->set_error_delimiters($this->config->item('field_error_start_tag'), $this->config->item('field_error_end_tag'));
				...
				$this->form_validation->set_rules('Fname', 'Firstname','trim|xss_clean|max_length[15]');
				$this->form_validation->set_rules('Lname', 'Lastname','trim|xss_clean|max_length[15]');
				...
				//$this->form_validation->set_rules('emergency_phone', 'Phone', 'required|trim|xss_clean|numeric|min_length[10]|max_length[10]');
...
function phone_code()
{
	$result = $this->Common_model->getTableData('country',array('id'=>$this->input->post('id')));
	if($result->num_rows() != 0)
    {
	echo $result->row()->phonecode;
	}
	else
		{
			$result = $this->Common_model->getTableData('country',array('country_name'=>$this->session->userdata('locale_country')));
			if($result->num_rows() != 0)
   			 {
				echo $result->row()->phonecode;
			 }
...
public function photo($id = "")
    {
        ...
        require_once APPPATH.'libraries/cloudinary/autoload.php';
                \Cloudinary::config(array( 
						...
						$email_id = $this->db->where('id',$id)->get('users')->row()->email;
			...
			//print_r($secureimage); exit;
			
			$split = explode('.', $secureimage);
         ...
         $ext= '.'.$split[3];
            ...
            if($secureimage != '')
            {
				...
				$image=$this->db->where('id',$id)->or_where('email',$email_id)->from('profile_picture')->get()->num_rows();
				...
				if($image == 0){
				$this->db->query('update users set photo_status = 1 where id = '.$id);	
				$this->db->insert('profile_picture',array('src'=>$src,'ext'=>$ext,'email'=>$email_id,'id'=>$id));
				}
				else if(!$image== 0){
				$this->db->where('email',$email_id);
				$this->db->update('profile_picture',array('src'=>$src,'ext'=>$ext));
...
public function references($param = '')
	{  
		//	if( ($this->dx_auth->is_logged_in()) || ($this->facebook_lib->logged_in()) || ($this->twconnect->logged_in()))
		if( ($this->dx_auth->is_logged_in()) || ($this->facebook_lib->logged_in()))
			{
		 	$this->load->library('email');
	   ...
	   if($this->input->post())
	   {
				...
				else { $email_name  = 'user_reference_'.$session_lang; }
...
function reference_req_approval()
{
	...
	if ($this->dx_auth->is_logged_in() || autologin() == 1)
	{
		$route['users/profile/(:num)']  = "users/vouch";
     	$route['users/profile']  = "users/vouch";  
		...
		if($this->form_validation->run())
	  	{	
					...
					if($data1['recommends_check']->num_rows() != 0)
					{
						...
						else { $email_name = 'reference_receive_'.$session_lang; }
...
$this->db->set('refer_userid',$refer_userid_ref)->where('id',$this->dx_auth->get_user_id())->update('users');
			...
			$admin_email = $this->dx_auth->get_site_sadmin();
				...
				if($session_lang == "") {
				$email_name = 'User_join_to_Referal_mail'; }
				else { $email_name = 'User_join_to_Referal_mail_'.$session_lang; }
...
if ($this->dx_auth->is_logged_in() || autologin() == 1)
		...
		if($this->input->post())
		{
			if ( !$this->dx_auth->is_logged_in())
			{
				...
				if($this->form_validation->run())
				{
					...
					if ($this->dx_auth->login(strtolower($username), $password, $remember_me))
					{
					...
					$this->Common_model->updateTableData('login_history',0,array('session_id'=>$this->session->userdata('session_id')),array('user_id'=>$this->dx_auth->get_user_id()));
					$this->db->where('id',$this->dx_auth->get_user_id())->update('users',array('last_login'=>time()));							
	...
	function signin_popup()
	{
	        ...
	        $data['fb_app_id'] = fb_api;
			$data['google_app_id'] = google_client_id;
			echo $this->load->view(THEME_FOLDER.'/users/view_signin_popup',$data);
	}
	function signup_popup()
	{
		...
		$data["meta_description"]    = get_meta_details('Sign_In / Sign_up','meta_description');
	   	$data['fb_app_id'] = fb_api;
		$data['google_app_id'] = google_google_client_id;
		echo $this->load->view(THEME_FOLDER.'/users/view_signup_popup',$data);
	}
	...
	public function CheckUserDetails()
	{
		...
		$this->form_validation->set_rules('Username', 'Username', 'trim|xss_clean|valid_email');	
		
			      if($this->form_validation->run())
	  	        {
	  	        	$query = $this->Users_model->get_user_by_email($Username);
				} else{
					$query = $this->Users_model->get_user_by_username($Username);
				}     		
			//echo $type;exit;
		//$query = $this->Users_model->get_user_by_username($Username);	
		//print_r($query);
		if($query->num_rows() == 1)
		{
			...
			//print_r(crypt($password, $stored_hash));exit;
	...
	function user_name_check()
	{
		...
		if ($this->dx_auth->is_username_available($uname))
		{
			echo "true";				
		} 
		else 
	...
	function emailcheck()
    {
         ...
         if ($this->dx_auth->is_email_available($uemail))
		{
			echo "true";			
		} 
		else 
     ...
     function cancel_account() {
					...
					// if( ($userto1==$userid || $userby1==$userid) && ($status1==0|| $status1==1|| $status1==2||$status1==3||$status1==4 || $status1==5|| $status1==6 || $status1==7|| $status1==11 || $status1==12||$status1==13)) 
			          if( ($userto1==$userid || $userby1==$userid) && ($status1==0|| $status1==1|| $status1==2||$status1==3||$status1==4 || $status1==5|| $status1==6 || $status1==7|| $status1==11 || $status1==12||$status1==13) && ($pay==0)  )
			          {
					       ...
					       $data =array(
					       	   'user_id' 				=> $userid,
					       	   'username'				=> get_user_by_id($userid)->username, 
					       	   'email'					=> get_user_by_id($userid)->email,
				  ...
				  //mail send functiopn start
					...
					//calender status
					 ...
					 //profiles
					  ...
					  //users  
					$this->Common_model->updateTableData('list',0,array('user_id'=>$this->dx_auth->get_user_id()),array('status'=>'0','is_enable'=>'0'));
					
					$this->Common_model->updateTableData('users',0,array('id'=>$this->dx_auth->get_user_id()),array('banned'=>'1'));
	...
	function change_password()
	{
		...
		{			
			...
			$val->set_rules('new_password', 'New Password', 'required|trim|xss_clean|min_length['.$this->min_password.']|max_length['.$this->max_password.']|callback_valid_case_insensitive_check_new_password|callback_check_new_password|matches[confirm_new_password]');
	...
	if($result->num_rows() != 0)
	{
		if($old_pass == 1)
		{
			...
			$this->db->where('id',$this->dx_auth->get_user_id())->update('users',array('password'=>$new_pass));
				...
				else { $email_name  = 'reset_password_'.$session_lang; }
		 ...
		 {	
				...
				else
				{
					...
					{						
						...
						else { $email_name = 'forgot_password_'.$session_lang; }
...
public function Twitter_MailId(){
	...
	$twitter_id =  $this->twconnect->tw_user_id;	
	  ...
	  $username = $this->twconnect->tw_user_name;
		$this->twconnect->twaccount_verify_credentials();
		$user_info = $this->twconnect->tw_user_info;
		 $mailId=$user_info->email;
	  
	  //$mailId  = $this->input->post('email');
	  	  
	 // $username  = $this->input->post('username');
	
	  $user_id = $this->Users_model->createTwitterUserEmail($twitter_id,$username,$mailId);
...
$this->db->set('ref_rent',$rent)->where('twitter_id',$twitter_id)->update('users');
 ...
 public function twitter_exists_email($id)
		{
...
/*else if($this->google_connect($user,$email)){
	
	
	$user_info = $this->Users_model->getUserInfobyGoogleEmail_id($email);
...
public function google_connect($email){
	
	$query=$this->db->get_where('users', array('google_email' => $email));
		if($query->num_rows==0){
		...
		if($query->row()->google_verify == 'yes' ){
...
public function verify()
{
	...
	//	 $this->session->set_flashdata('flash_message', $this->Common_model->flash_message('error','Email Address Not Verified.'));
	  	...
	  	$user_id    = $this->dx_auth->get_user_id();	
			...
			$rows = $this->Common_model->getTableData('users', array('id' => $user_id))->row();
		$data['view']=$rows->file1;
		
		$rows = $this->Common_model->getTableData('users', array('id' => $user_id))->row();
		$data['viewf']=$rows->file2;
		
		
	    $id=$this->dx_auth->get_user_id();
		$data['status']=$this->Contacts_model->get_status($id);
...
public function google_verify()
{
	    ...
	    $result = $this->db->where('id',$this->dx_auth->get_user_id())->from('users')->get();
			  if($result->num_rows()==1)
			  {
			  	...
			  	$this->db->where('id',$this->dx_auth->get_user_id())->update('users',array('google_verify'=>'yes','google_email'=>"$email",'google_id'=>$google_id));
...
public function facebook_verify()
{
	...
	$result = $this->db->where('id',$this->dx_auth->get_user_id())->from('users')->get();
			  if($result->num_rows()!=0)
			  {	  	
				$this->db->where('id',$this->dx_auth->get_user_id())->update('users',array('facebook_verify'=>'yes','facebook_email'=>"$email", 'fb_id'=>$fb_id));
				echo 'verified';			  	
				}
...
public function email_verify()
{
	...
	$admin_email = $this->dx_auth->get_site_sadmin();
		...
		else {$email_name   = 'email_verification_'.$session_lang; }
...
public function edit_email_verify($email)
{
	...
	$admin_email = $this->dx_auth->get_site_sadmin();
		...
		$data['get'] = $this->db->where('id',$this->dx_auth->get_user_id())->update('profiles',array('tmail'=>$toEmail));
...
$username = $this->db->where('id',$this->dx_auth->get_user_id())->from('users')->get()->row()->username;
		...
		$this->session->set_flashdata('msg','Your Email  ');
				$this->session->set_flashdata('msgs','is pending for verification.  ');
...
$this->Common_model->updateTableData('users',0,array('id'=>$this->dx_auth->get_user_id()),array('phone_verification_code'=>$data));
$this->Common_model->updateTableData('profiles',0,array('id'=>$this->dx_auth->get_user_id()),array('phnum'=>trim($this->input->post('phone_number'))));	
	//exit;
	$from_phone_number = $this->db->get_where('settings', array('code' => 'NEXMO_API_PHONE_NO'))->row()->string_value;
       
	    $from = $from_phone_number;
        $to = substr($this->input->post('phone_number'),1);
       // $to = 919629956100;
        $message = array(
            'text' => "Please enter code '".$data."' on ".$this->dx_auth->get_site_title()." to verify."
        );
...
{
	$this->Common_model->updateTableData('profiles',0,array('id'=>$this->dx_auth->get_user_id()),array('phnum'=>''));
	$this->Common_model->updateTableData('users',0,array('id'=>$this->dx_auth->get_user_id()),array('phone_verify'=>'no'));
	echo 'removed';exit;
}

function check_mobile_verification()
{
	$result = $this->Common_model->getTableData('users',array('id'=>$this->dx_auth->get_user_id(),'phone_verification_code'=>$this->input->post('phone_code')));
	
	if($result->num_rows() != 0)
	{
		$this->Common_model->updateTableData('users',0,array('id'=>$this->dx_auth->get_user_id()),array('phone_verify'=>'yes'));
		echo 'success';exit;
	}
	else {
		$this->Common_model->updateTableData('users',0,array('id'=>$this->dx_auth->get_user_id()),array('phone_verify'=>'no'));
...
{
	echo $this->Common_model->getTableData('profiles',array('id'=>$this->dx_auth->get_user_id()))->row()->phnum;exit;
}
...
public function passport(){
	...
	$user_id    = $this->dx_auth->get_user_id();
	$this->form_validation->set_rules('file1', 'file1', 'trim|xss_clean');
	$this->form_validation->set_rules('file2', 'file2', 'trim|xss_clean');
				
	if($this->form_validation->run())
    {		
          $config['upload_path'] = './verify/';
		  $config['allowed_types'] = 'pdf|png|jpg|jpg';
...
$status = 0;
		   ...
		   {									
		      $res = $this->upload->data('file1');
			  $data['file1']=$res['file_name'];
			$status = 1;   
			 $this->Common_model->passport($data,$user_id);
		   }
		   else {
			  $error = $this->upload->display_errors();
			  $this->session->set_flashdata('flash_message', $this->Common_model->flash_message('error',$error));
			  $status = 0;
		}
           if($this->upload->do_upload('file2'))
		   {
	          $res1 = $this->upload->data('file2');
		      $data['file2']=$res1['file_name'];
			  $status = 1;
			  $this->Common_model->passport($data,$user_id);
		   } 
        else {
        	$status = 0;
			   $errorf = $this->upload->display_errors();
			   $this->session->set_flashdata('flash_message', $this->Common_model->flash_message('error',$errorf));
			   
		}
		if($status == 0)
		{
		$this->session->set_flashdata('flash_message', $this->Common_model->flash_message('error','Files are not uploaded properly'));
			
		}else{
		$this->session->set_flashdata('flash_message', $this->Common_model->flash_message('success','Files upload successfully'));
...
public function view_file(){
         	$user_id    = $this->dx_auth->get_user_id();
			$data['result']  =  $this->Common_model->view_file($data,$user_id);
			$data['message_element']     = "users/view_verify";
		$this->load->view('template',$data);
...
public function delete(){
	$user_id    = $this->dx_auth->get_user_id();
	$target_path = realpath(APPPATH . '../verify/');	
		$rows = $this->Common_model->getTableData('users', array('id' => $user_id))->row();
		$var=$rows->file1;
		$this->Common_model->delete_file($user_id);
		unlink($target_path.$var);
		//echo($result);
		//exit;
	$this->session->set_flashdata('flash_message', $this->Common_model->flash_message('success','File has been deleted successfully'));
	redirect('users/verify');
	$data['message_element']     = "users/view_verify";
		$this->load->view('template',$data);
}
public function passport_verification()
{
	...
	$session_lang = $this->session->userdata('locale');
	if($session_lang == "") {
		$email_name   = 'passport_verification';} 
	else {$email_name   = 'passport_verification_'.$session_lang;}
	
	$fromName     = $this->dx_auth->get_site_title();
    $username = $this->db->where('id',$this->dx_auth->get_user_id())->from('users')->get()->row()->username;
    $user_id=$this->dx_auth->get_user_id();
   
	$splVars = array("{user_id}" => $user_id,"{user_name}" => $username);
													
	$res=$this->Email_model->sendMail($toEmail,$fromEmail,$fromName,$email_name,$splVars);
	if(!$res)
		{
			$this->session->set_flashdata('flash_message', $this->Common_model->flash_message('success',translate('ID Verification Sent to admin.')));
			redirect('users/verify');
		}
    else {
	$this->session->set_flashdata('flash_message', $this->Common_model->flash_message('error',translate('Passport Verification cannot Sent to email.')));
	redirect('users/verify');
    }	
...
function get_userlist()
{
	...
	$image_datas = array();
	$id_datas =array();
	$res = $this->db->select('id,username,email')->where('banned',0)->like('username',trim($user_name_or_email),'after')->get('users');
	if($res->num_rows() == 0)
	{
	$res = $this->db->select('id,username,email')->where('banned',0)->where('email',trim($user_name_or_email))->get('users');		
	}
	$tCount = $res->num_rows() ;
	if($res->num_rows() > 0)
	{
	...
	foreach($res->result() as $row){
		...
		$properties .= '{
			...
			"user_image":"'.$this->Gallery->profilepic($row->id,3).'",
			"user_id":"'.$row->id.'"
			}'.$comma;
...
public function deletef(){
	...
	$target_path = realpath(APPPATH . '../verify/');	
		...
		$rowsv = $this->Common_model->getTableData('users', array('id' => $user_id))->row();
		$varf=$rowsv->file2;
		$this->Common_model->del_file($user_id);
		
		unlink($target_path.$varf);

...
public function blocked_calendar()
	{
				extract($this->input->post());
				$dat_block = array();
				$result_block = $this->db->where('list_id',$list_id)->get('calendar');
				if($result_block->num_rows() > 0)
				{
				foreach($result_block->result() as $row_block)
				{	
					$book1 = $row_block->booked_days;
					$book = get_local_time($book1);
					$date_ = date('m/d/Y',$book);
					$dat_block[]=$date_;
				}
				}
				echo json_encode($dat_block);	
	}
	

public function specific_dates(){	
			 		extract($this->input->post());
					
					$booked_from = $start_date;
					$booked_to   = $end_date;
				
					$group_id    = $this->db->select_max('group_id')->get('calendar')->row()->group_id;
					...
					$insertData['list_id'] = $list_id;
						$this->db->where('list_id',$list_id)->where('availability','Available')->delete('calendar');
						...
						$check = $this->db->where('list_id',$list_id)->where('booked_days',$insertData['booked_days'])->get('calendar');
						...
						if($thoseDays==1){
							echo 'Those some days you specified already listed be Booked.';
						} else {
							echo 'You specified days listed be Booked.';
						}
...
public function block_dates(){	
					...
					$insertData['availability']= 'Available';
					$insertData['list_id'] = $list_id;												
					$insertData['booked_days'] = get_gmt_time(strtotime($book));
					$insertData['created']     = local_to_gmt();							
			        
		
		$temp_id = $insertData['list_id'];			
		$temp = $insertData['booked_days'];			
		$result = $this->db->where('list_id',$temp_id)->get('calendar');
		foreach($result->result() as $row)
			{
	
				$booked = $row->booked_days;
...
{	
					...
					$list_id = $list_id;												
					$booked_days = get_gmt_time(strtotime($book));
						
												
$this->db->where('list_id',$list_id)->where('booked_days',$booked_days)->delete('calendar');
...
public function get_dates()
{
	...
	extract($this->input->post());
	$id=$room_id;
	$result = $this->db->where('list_id',$id)->get('calendar');
	foreach($result->result() as $row)
	{	
	$book1 = $row->booked_days;
	$book = get_local_time($book1);
	$date = date('n/j/Y',$book);
	echo $date.",";
	}	
   ...
   $data['list_title']  = $row->title;
	...
	$min_currency= round(get_currency_value_lys($from,$to,$amount));	echo $min_currency; */
					...
					$res = $this->db->where('id',$id)->from('list')->get()->row();
					$cur_code = $res->currency;
					...
					if($availability=="Available")					
						{
							...
							$res=$query->result_array();
							$query_list=$this->Common_model->getTableData('list', array( 'id' => $pdata['list_id']));
							$res_list=$query_list->result_array();
							$listprice = $res_list['price'];
							$count=1;
							...
							foreach($res as $row)
							{
								...
								if($availability=="Available" && $pricevalue == "") {
										print_r($currency_value); exit;						
										$condition = array('list_id' => $pdata['list_id'], 'start_date' => $from, 'end_date' => $to);	
										$updatedata=array('price' => $currency_value);	

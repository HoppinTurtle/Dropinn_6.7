...
class Lists extends CI_Controller
{
	...
	function index()
	{
		...
		$query = $this->db->order_by('id','desc')->get('list');
		// Get all lists
		...
		// Get all lists
		$query = $this->db->order_by('id','desc')->where('created >=',$created)->get('list');		
		$data['users'] = $query->result();				
				}
...
\Cloudinary::config(array( 
  ...
  "api_secret" => cloud_s_key
   ...
   $data['list_title']  = $row->title;
        ...
        foreach($check as $id)
        {
        $reservation_status=$this->Common_model->getTableData( 'reservation', array('list_id' => $id ));
        ...
        $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success',translate_admin('Rooms Deleted Successfully')));
        redirect_admin('lists');
        }
...
//                         
                    ...
                    // }
   // else if(count($_FILES["userfile"]["error"]) == 1) {
    // if($_FILES["userfile"]["name"][0] != '')addlist
    // {
    // $rimages = $this->Gallery->get_imagesG($listId);
                    // $i = 1;
                    // $replace = '<ul class="clearfix">';
...
extract($this->input->get());
	/*$query = $this->db->query("SELECT list.id as List_ID, list.title as List_Title, users.username as Added_By, list.price as Price, list.currency as List_Currency, list.address as Address, list.room_type as Home_Type, list.capacity as Capacity, FROM_UNIXTIME(list.created, '%b %d, %Y') as Created_Date, CASE WHEN list.status = 1 AND list.is_enable = 1 THEN 'Listed' ELSE 'UnListed' END AS Status FROM list INNER JOIN users ON list.user_id = users.id");*/	    			
    $query = $this->db->query("SELECT list.id as List_ID, list.title as List_Title, users.username as Added_By, list.price as Price, list.currency as List_Currency, list.address as Address, list.room_type as Home_Type, list.capacity as Capacity, list.min_stay as Minimum_Stay, list.max_stay as Maximum_Stay, FROM_UNIXTIME(list.created, '%b %d, %Y') as Created_Date, CASE WHEN list.status = 1 AND list.is_enable = 1 THEN 'Listed' ELSE 'UnListed' END AS Status FROM list INNER JOIN users ON list.user_id = users.id");
  ...
  {   
   if(isset($export))
   {
   	$data = $query;
		...
		$name='Export-'.$site_name.'_list_details.csv';
		force_download($name, $query);
   } 
    }else{
  			 $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success',translate_admin("User's listing not found!")));
...
extract($this->input->get());
    ...
    $query = $this->db->query("SELECT id as User_ID, username as User_Name, email as Email, CASE WHEN facebook_verify = 1 AND google_verify = 1 AND email_verify = 'yes' AND phone_verify = 'yes' THEN 'Yes' ELSE 'No' END AS Verified, FROM_UNIXTIME(created, '%b %d, %Y') as Created_Date, CASE WHEN banned = 1 THEN 'Inactive' ELSE 'Active' END AS Status FROM users");
	   if($query->num_rows()>0)
    {  
   if(isset($export))
   {
   	$data = $query;
		...
		$query=$this->dbutil->csv_from_result($data, $delimiter, $newline); 
		$site_name  = $this->dx_auth->get_site_title();
		$name='Export-'.$site_name.'_User_details.csv';
		force_download($name, $query);
   }
    }else{
   			 $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success',translate_admin('Members not found!')));
			redirect_admin('backend');
    }
				 			
}

public function export_superhost()
{
...
extract($this->input->get());
    ...
    $query = $this->db->query("SELECT id as User_ID, username as User_Name, email as Email, CASE WHEN facebook_verify = 1 AND google_verify = 1 AND email_verify = 'yes' AND phone_verify = 'yes' THEN 'Yes' ELSE 'No' END AS Verified, FROM_UNIXTIME(created, '%b %d, %Y') as Created_Date, CASE WHEN banned = 1 THEN 'Inactive' ELSE 'Active' END AS Status FROM users where superhost=1");
       ...
       if(isset($export))
   {
   	$data = $query;
		...
		force_download($name, $query);
			 ...
			 $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success',translate_admin('Super Host not found!')));
			redirect_admin('backend');
			    }	  
...
public function delete_cal($param='')
   ...
   redirect('administrator/lists/managelist/'.$ical_urls->row()->list_id.'?month=1');
	...
	public function change_user($room_id, $user_id){
				
			$check_contact = $this->db->where('list_id',$room_id)->get('contacts');
			$check = $this->db->where('list_id',$room_id)->get('reservation');
			
			if($check->num_rows() > 0 || $check_contact->num_rows() > 0){
			//$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error',"This parking space has at least one ongoing contact requests/reservation."));
			echo "fail"; exit ; 
			}else{	
			$data['user_id'] = $user_id;
			$this->db->where('id',$room_id)->update('list',$data);
			$this->db->where('id',$room_id)->update('lys_status',$data);
			$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success',translate_admin('User assigned successfully')));
			}
 ...
 function generate_Excel($xl_type){
 	
	$property = $this->db->order_by('id','asc')->get('property_type');
	
	$currency = $this->db->order_by('id','asc')->get('currency');
 	//load our new PHPExcel library
$this->load->library('Excel');
require_once("app/third_party/PHPExcel/NamedRange.php");
//activate worksheet number 1
$this->excel->setActiveSheetIndex(0);
//name the worksheet
$this->excel->getActiveSheet()->setTitle('Sample format');

$this->excel->getActiveSheet()->setCellValue('a1', "title");

$this->excel->getActiveSheet()->setCellValue('b1', "desc");
$this->excel->getActiveSheet()->setCellValue('c1', "price");
$this->excel->getActiveSheet()->setCellValue('d1', "currency");

$this->excel->getActiveSheet()->setCellValue('e1', "instance_book");
$this->excel->getActiveSheet()->setCellValue('f1', "property_id");
$this->excel->getActiveSheet()->setCellValue('g1', "room_type");
$this->excel->getActiveSheet()->setCellValue('h1', "beds");
$this->excel->getActiveSheet()->setCellValue('i1', "bedrooms");
$this->excel->getActiveSheet()->setCellValue('j1', "bathrooms");
$this->excel->getActiveSheet()->setCellValue('k1', "bed_type");
$this->excel->getActiveSheet()->setCellValue('l1', "address");

$this->excel->getActiveSheet()->setCellValue('m1', "country");
$this->excel->getActiveSheet()->setCellValue('n1', "city");
$this->excel->getActiveSheet()->setCellValue('o1', "state");
$this->excel->getActiveSheet()->setCellValue('p1', "zip_code");
$this->excel->getActiveSheet()->setCellValue('q1', "lat");
$this->excel->getActiveSheet()->setCellValue('r1', "long");
$this->excel->getActiveSheet()->setCellValue('s1', "min_stay");
$this->excel->getActiveSheet()->setCellValue('t1', "max_stay");
$this->excel->getActiveSheet()->setCellValue('u1', "is_enable");
$this->excel->getActiveSheet()->setCellValue('v1', "status");
$this->excel->getActiveSheet()->setCellValue('w1', "is_featured");

// For Property type
$this->excel->setActiveSheetIndex(0);

$objValidation = $this->excel->getActiveSheet()->getCell('f2')->getDataValidation();
...
$objValidation->setFormula1('Propertytype!$A$1:$A$'.$property->num_rows());

$this->excel->createSheet();
$this->excel->setActiveSheetIndex(1);
$this->excel->getActiveSheet()->setTitle('Propertytype');


$j=1;
foreach($property->result() as $property_){
    $this->excel->getActiveSheet()->setCellValue("A{$j}", "{$property_->id}");
	 $this->excel->getActiveSheet()->setCellValue("B{$j}", "{$property_->type}");
	 $j++;
}
	  $this->excel->addNamedRange( 
        new PHPExcel_NamedRange('property',$this->excel->getSheetByName('Propertytype'),'$A$1:$A$'.$property->num_rows()) 
    );
//Property type ends
...
$objValidation = $this->excel->getActiveSheet()->getCell('g2')->getDataValidation();
$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
$objValidation->setAllowBlank(false);
$objValidation->setShowInputMessage(true);
...
$objValidation->setPromptTitle('Pick from list');
$objValidation->setPrompt('Please pick a value from the drop-down list.');
$objValidation->setFormula1('Roomtype!$A$1:$A$3');

$this->excel->createSheet();
$this->excel->setActiveSheetIndex(2);
$this->excel->getActiveSheet()->setTitle('Roomtype');
$this->excel->getActiveSheet()->setCellValue("A1", "Entire Home/Apt");
$this->excel->getActiveSheet()->setCellValue("A2", "Private Room");
$this->excel->getActiveSheet()->setCellValue("A3", "Shared Room");
	  $this->excel->addNamedRange( 
        new PHPExcel_NamedRange('roomtype',$this->excel->getSheetByName('Roomtype'),'$A$1:$A$2:$A$3') 
    );
//Room type ends 
...
$objValidation->setPromptTitle('Pick from list');
$objValidation->setPrompt('Please pick a value from the drop-down list.');
$objValidation->setFormula1('Currency!$A$1:$A$'.$currency->num_rows()); // Make sure to put the list items between " and "  !!!

$this->excel->createSheet();
$this->excel->setActiveSheetIndex(3);
$this->excel->getActiveSheet()->setTitle('Currency');

$i=1;
foreach($currency->result() as $currencies){
    $this->excel->getActiveSheet()->setCellValue("A{$i}", "{$currencies->currency_code}");
	 $i++;
}
//Currency ends 
...
$objValidation = $this->excel->getActiveSheet()->getCell('w2')->getDataValidation();
$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
$objValidation->setAllowBlank(false);
$objValidation->setShowInputMessage(true);
...
$objValidation->setPromptTitle('Pick from list');
$objValidation->setPrompt('Please pick a value from the drop-down list.');
$objValidation->setFormula1('Enable_Feature!$A$1:$A$2'); // Make sure to put the list items between " and "  !!!

$this->excel->createSheet();
$this->excel->setActiveSheetIndex(4);
$this->excel->getActiveSheet()->setTitle('Enable_Feature');
$this->excel->getActiveSheet()->setCellValue("A1", "Yes");
$this->excel->getActiveSheet()->setCellValue("A2", "No");

$this->excel->setActiveSheetIndex(0);

$objValidation = $this->excel->getActiveSheet()->getCell('k2')->getDataValidation();
$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
$objValidation->setAllowBlank(false);
$objValidation->setShowInputMessage(true);
...
$objValidation->setPromptTitle('Pick from list');
$objValidation->setPrompt('Please pick a value from the drop-down list.');
$objValidation->setFormula1('Bedtype!$A$1:$A$5'); // Make sure to put the list items between " and "  !!!

$this->excel->createSheet();
$this->excel->setActiveSheetIndex(5);
$this->excel->getActiveSheet()->setTitle('Bedtype');
$this->excel->getActiveSheet()->setCellValue("A1", "Airbed");
$this->excel->getActiveSheet()->setCellValue("A2", "Futon");
$this->excel->getActiveSheet()->setCellValue("A3", "Pull-out Sofa");
$this->excel->getActiveSheet()->setCellValue("A4", "Couch");
$this->excel->getActiveSheet()->setCellValue("A5", "Real Bed");

$this->excel->setActiveSheetIndex(0);

$objValidation = $this->excel->getActiveSheet()->getCell('u2')->getDataValidation();
$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
$objValidation->setAllowBlank(false);
$objValidation->setShowInputMessage(true);
...
$objValidation = $this->excel->getActiveSheet()->getCell('v2')->getDataValidation();
$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
$objValidation->setAllowBlank(false);
$objValidation->setShowInputMessage(true);
...
$objValidation->setPromptTitle('Pick from list');
$objValidation->setPrompt('Please pick a value from the drop-down list.');
$objValidation->setFormula1('Status!$A$1:$A$2'); // Make sure to put the list items between " and "  !!!

$this->excel->createSheet();
$this->excel->setActiveSheetIndex(6);
$this->excel->getActiveSheet()->setTitle('Status');
$this->excel->getActiveSheet()->setCellValue("A1", 0);
...
$objValidation = $this->excel->getActiveSheet()->getCell('s2')->getDataValidation();
$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
$objValidation->setAllowBlank(false);
$objValidation->setShowInputMessage(true);
...
$objValidation->setPromptTitle('Pick from list');
$objValidation->setPrompt('Please pick a value from the drop-down list.');
$objValidation->setFormula1('counts!$A$1:$A$15'); // Make sure to put the list items between " and "  !!!

$this->excel->createSheet();
$this->excel->setActiveSheetIndex(7);
$this->excel->getActiveSheet()->setTitle('counts');
for($i_mn_stat=1; $i_mn_stat<=15; $i_mn_stat++)
$this->excel->getActiveSheet()->setCellValue("A".$i_mn_stat, $i_mn_stat);
...
$objValidation = $this->excel->getActiveSheet()->getCell('t2')->getDataValidation();
$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
$objValidation->setAllowBlank(false);
$objValidation->setShowInputMessage(true);
...
$objValidation = $this->excel->getActiveSheet()->getCell('h2')->getDataValidation();
$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
$objValidation->setAllowBlank(false);
$objValidation->setShowInputMessage(true);
...
$objValidation = $this->excel->getActiveSheet()->getCell('i2')->getDataValidation();
$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
$objValidation->setAllowBlank(false);
$objValidation->setShowInputMessage(true);
...
$objValidation = $this->excel->getActiveSheet()->getCell('j2')->getDataValidation();
$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
$objValidation->setAllowBlank(false);
$objValidation->setShowInputMessage(true);
...
$objValidation->setPromptTitle('Pick from list');
$objValidation->setPrompt('Please pick a value from the drop-down list.');
$objValidation->setFormula1('counts_decimal!$A$1:$A$16'); // Make sure to put the list items between " and "  !!!


$this->excel->createSheet();
$this->excel->setActiveSheetIndex(8);
$this->excel->getActiveSheet()->setTitle('counts_decimal');
$i_cnt_temp=1;
for($i_cnt=0.5; $i_cnt<=8; $i_cnt+=0.5)
{
$this->excel->getActiveSheet()->setCellValue("A".$i_cnt_temp, $i_cnt);
$i_cnt_temp++;
}
...
$filename='Sample.'.$xl_type; //save our workbook as this file name
if($xl_type == "xls"){
...
$objWriter->save('php://output');
 ...
 public function import_xls()
 {
        ...
        if (!$this->upload->do_upload()) 
         {
        	//var_dump($this->upload->data());
             $data['error'] = $this->upload->display_errors();
 			 $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error',$data['error']."Type==".var_dump($typ)));
			 redirect_admin('lists');
            
        } else if($this->input->post('user_id_list_2') == "")
		{
		 	$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error',"You should need to select a username to import!"));
			redirect_admin('lists');	
		} else {
  ...
  $this->load->library("Excel");
  PHPExcel_Settings::setZipClass(PHPExcel_Settings::PCLZIP); // Add this line if php zip module not enabled 
  
  $inputFileType = PHPExcel_IOFactory::identify($file_path);
  //OR use Excel2007 directly
  ...
  // To get the array value from Excel data
  $highestRow = $objWorksheet->getHighestRow();
			...
			$namedDataArray[$i]["user_id"] = $this->input->post('user_id_list_2');  // Default Admin user		$namedDataArray[$i]["created"] = time();
			$namedDataArray[$i]["cancellation_policy"] = 1;
			$namedDataArray[$i]["list_pay"] = 0;
			$namedDataArray[$i]["calendar_type"] = 1;
			$namedDataArray[$i]["street_address"] = $namedDataArray[$i]["address"];	
		 	unset($namedDataArray[$i]['id']); // Reomve primary key
					
					if($namedDataArray[$i]['is_enable'] == "Yes"){
						$namedDataArray[$i]['is_enable'] = 1 ;
					}else{
						$namedDataArray[$i]['is_enable'] = 0 ;
					}
					
					if($namedDataArray[$i]['is_featured'] == "Yes"){
						$namedDataArray[$i]['is_featured'] = 1 ;
					}else{
						$namedDataArray[$i]['is_featured'] = 0 ;
					}
					if($namedDataArray[$i]['status'] == 0){
						$namedDataArray[$i]['status'] = 0 ;
					}else{
						$namedDataArray[$i]['status'] = 1 ;
					}
					

					if($namedDataArray[$i]['price']!="" && $namedDataArray[$i]['currency']!="")
					{
					   if($namedDataArray[$i]['min_stay']>$namedDataArray[$i]['max_stay'])
					   {
					   	 $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error',translate_admin('minimum and maximum stay values are Invalid. Please recheck the uploaded file ')));
						 redirect_admin('lists');	
				       }	
					//$this->db->insert('list',array_filter($namedDataArray[$i])); // Insert into list table
					$this->db->insert('list',$namedDataArray[$i]); // Insert into list table
				  
					 $price_status = 1;
					if($namedDataArray[$i]['title'] != "" && $namedDataArray[$i]['desc'] != "" )
					 {
					  $overview_status = 1;	
					 }else{
					  $overview_status = 0;	
					 }
					 if($namedDataArray[$i]['property_id'] != "" && $namedDataArray[$i]['room_type'] != "" && $namedDataArray[$i]['bed_type'] != "")
					 {
					  $list_status = 1;	
					 }else{
					  $list_status = 0;	
					 }
					 if($namedDataArray[$i]['address'] != "")
					 {
					 	$addr_status = 1;
					 }else{
					 	$addr_status = 0;
					 }
				 // Insert into lys_status table
					 $insertid = $this->db->insert_id();
					 $this->db->insert('lys_status',
					 array(
					 'id'=>$insertid,
					 'user_id'=>$this->input->post('user_id_list_2'),
					 'calendar'=>1,
					 'price'=>$price_status,
					 'title'=>1,
				  ...
				  // Insert into price table
					  ...
					  array(
					 'id'=>$insertid,
					 'night'=> $namedDataArray[$i]['price'],
					 'week'=> $namedDataArray[$i]['price']*7,
					 'month'=> $namedDataArray[$i]['price']*30,
					 'guests'=> 1,
					 'currency'=> $namedDataArray[$i]['currency']
					  ));
					  }
				else{
				$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error',translate_admin('Sorry, Missing some values. Please recheck the uploaded file ')));
				redirect_admin('lists');		
					}		 
				//echo"<pre>"; print_r($namedDataArray[$i]);
				$i++; 
            ...
            /*$this->db->select('*');
			$this->db->from('list');
			$this->db->where('user_id',$this->input->post('user_id_list_2'));
			$this->db->order_by('id', 'desc');
			$this->db->limit($i);
			
			$qry = $this->db->get();
			$result['users'] = $qry->result();
			$result['message_element'] = "administrator/view_lists";*/
            $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success',translate_admin('Lists imported successfully')));
			redirect_admin('lists');
			
			//$this->load->view('administrator/admin_template', $result);

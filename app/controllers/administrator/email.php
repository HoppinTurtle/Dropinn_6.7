	...
	// Get offset and limit for page viewing
	 ...
	 // Number of record showing per page
		...
		if($start > 0)
		 ...
		 // Get all users
		$data['email_settings'] = $this->db->get('email_templates', $row_count, $offset);
		
		$p_config['base_url']    = admin_url('email/index');
	...
	public function settings()
	{
				...
				if($data_value == 5 )
		        {
	              //  $this->form_validation->set_rules('smtp_domain','Domain name','required');		
	                ...
	                $this->form_validation->set_rules('smtp_do_port','Mail Port','required');							  
					    ...
					    if($this->form_validation->run())
					    {
								...
								$data17['value']    = $this->input->post('smtp_do_port');
								$this->db->where('code', 'MAIL_DOMAIN_PORT');
								$this->db->update('email_settings',$data17);
                 ...
                 else
	             {
					...
					//exit;
							...
							$query8                  = $this->db->get_where('email_settings', array('code' => 'MAIL_DOMAIN_PORT'));
							$data['smtp_do_port']     = $query8->row()->value;
		//$this->load->view('administrator/email/settings',$data);
		        ...
		        {
	                $this->form_validation->set_rules('smtp_user','SMTP User name','required');		
			...
			else
			{
				...
				//if($this->form_validation->run())
			     ...
			     /*	$query6                  = $this->db->get_where('email_settings', array('code' => 'DOMAIN_NAME'));
					...
					$query11                  = $this->db->get_where('email_settings', array('code' => 'MAIL_DOMAIN_PORT'));
					$data['smtp_do_port']     = $query11->row()->value;
				 	//$this->load->view('administrator/email/settings',$data);
				       $data['message_element'] = "administrator/email/settings";
					//print_r($data);
					//exit;
...
$data['result'] = $this->common_model->user_notification();

		
		
					       $data['message_element'] = "administrator/email/user_notification";

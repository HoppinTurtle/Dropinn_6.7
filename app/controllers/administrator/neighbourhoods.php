...
//             
             $updateKey                             = $this->uri->segment(4);
                ...
                if($edit_post->num_rows()!=0)
                {
                  ...
                  // $upload_data = $this->upload->data();                 
                  $updateData['image_name']    =  $_FILES["place_image"]['name'];
...
array("public_id" => "images/neighbourhoods/".$city_id."/".$place_id."/".$name3
));
// print_r($image);
}catch (Exception $e) {
$error = $e->getMessage();
...
function editpost()
	{
		...
		if($this->input->post('submit'))
		{
			...
			$place_id = $this->db->where('place_name',$place_name)->get('neigh_city_place')->row()->id;
				...
				$updateKey 							= $this->uri->segment(4);
                  ...
                  "0" => $_FILES["big_image1"]["tmp_name"],
                   "1" => $_FILES["small_image1"]["tmp_name"],
                      "2" => $_FILES["small_image2"]["tmp_name"],
                       "3" => $_FILES["small_image3"]["tmp_name"],
                        "4" => $_FILES["small_image4"]["tmp_name"],
                         "5" => $_FILES["small_image5"]["tmp_name"],
                          "6" => $_FILES["big_image2"]["tmp_name"],
                    		"7" => $_FILES["big_image3"]["tmp_name"],        
                  );
                  ...
                  "0" => $_FILES["big_image1"]["name"],
                    "1" => $_FILES["small_image1"]["name"],
                      "2" => $_FILES["small_image2"]["name"],
                       "3" => $_FILES["small_image3"]["name"],
                        "4" => $_FILES["small_image4"]["name"],
                         "5" => $_FILES["small_image5"]["name"],
                          "6" => $_FILES["big_image2"]["name"],
                    		"7" => $_FILES["big_image3"]["name"],        
                  );
                  ...
                  $arraye = array_combine($images, $imagesname);
				$onErr = $i= 0 ;
				$name_arr = array_combine($imagesname,$InArray);
		...
		$temp2 = explode('.', $name);
		 $ext2  = array_pop($temp2);
			$name3 = implode('.', $temp2);
				...
				// print_r($image);
				$image_data[$name_arr[$name]]  = $this->remove_space($name);
				$this->db->where('id',$updateKey)->update('neigh_post', $image_data);
				}catch (Exception $e) {
				$error = $e->getMessage();
				// print_r($error);
		...
		if($onErr == 1){
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error',translate_admin('Some files are not uploaded to server!')));
		redirect_admin("neighbourhoods/editpost/$id");
		}
		 		$data7['city']            = $city; 
				$data7['place']           = $place; 

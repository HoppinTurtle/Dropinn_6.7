	...
	function manage_gateway($id = '')
	{
		...
		else if($this->input->post('edit'))
		{
				...
				if(count($check) == 1)
				{
				 ...
				 $data['result']         = $query->row();
					...
					$data['merchantId']      = $this->db->get_where('payment_details', array('code' => 'BT_MERCHANT'))->row()->value;
					$data['publicKey']       = $this->db->get_where('payment_details', array('code' => 'BT_PUBLICKEY'))->row()->value;
					$data['privateKey']       = $this->db->get_where('payment_details', array('code' => 'BT_PRIVATEKEY'))->row()->value;
				    
				
				  	$data['SecretKey']      = $this->db->get_where('payment_details', array('code' => 'SecretKey'))->row()->value;
					$data['PublishableKey'] = $this->db->get_where('payment_details', array('code' => 'PublishableKey'))->row()->value;
					$data['LSecretKey']     = $this->db->get_where('payment_details', array('code' => 'LSecretKey'))->row()->value;
					$data['LPublishableKey']= $this->db->get_where('payment_details', array('code' => 'LPublishableKey'))->row()->value;
					//$data['pwd']      = $this->db->get_where('payment_details', array('code' => 'PWD'))->row()->value;
					$data['paypal_live']    = $this->db->get_where('payments', array('payment_name' => 'Paypal'))->row()->is_live;
					$data['stripe_live']    = $this->db->get_where('payments', array('payment_name' => 'Stripe'))->row()->is_live;
		...
		else if($this->input->post('update'))
		{
	      ...
	      $this->form_validation->set_error_delimiters('<p>', '</p>');
			...
			if($payId == 1)
			{
			$this->form_validation->set_rules('merchantId','Merchant ID','required|trim|xss_clean');
			...
			$this->form_validation->set_rules('SecretKey','Test SecretKey','required|trim|xss_clean');
			$this->form_validation->set_rules('PublishableKey','Test PublishableKey','required|trim|xss_clean');
			$this->form_validation->set_rules('LSecretKey','Live SecretKey','required|trim|xss_clean');
			$this->form_validation->set_rules('LPublishableKey','Live PublishableKey','required|trim|xss_clean');
			}
			else {
			$this->form_validation->set_rules('ventor_id','Ventor ID','required|trim|xss_clean');
			$this->form_validation->set_rules('security','Security(2Checkout Username)','required|trim|xss_clean');
			...
			{
			$data1['value']    = $this->input->post('SecretKey');
			$this->db->where('code', 'SecretKey');
			$this->db->update('payment_details',$data1);
			
			$data2['value']    = $this->input->post('PublishableKey');
			$this->db->where('code', 'PublishableKey');
			$this->db->update('payment_details',$data2);
			 
			$data1['value']    = $this->input->post('LSecretKey');
			$this->db->where('code', 'LSecretKey');
			$this->db->update('payment_details',$data1);
			
			$data2['value']    = $this->input->post('LPublishableKey');
			$this->db->where('code', 'LPublishableKey');
			$this->db->update('payment_details',$data2);
			
            $updateData['is_enabled'] = $this->input->post('is_active');
		    $updateData['is_live']    = $this->input->post('credit_url');
...
function finance()
	{
		...
		$data['result'] =  $this->Trips_model->get_reservation();
				if(($this->input->get()))
		{
		...
		$cond = array('reservation.book_date >='=>$created);
		$data['result'] =  $this->Trips_model->get_reservation($cond);
				}
	...
	function export_pend(){
		...
		$condition=array('is_payed'=>0,'is_payed_host'=>0,'is_payed_guest'=>0,'reservation.status'=>10);
		//$query          = $this->Trips_model->get_reservation($condition);
		$this->db->select("title, users.username,users.id,list.currency,list.price,reservation_status.name,");
		$this->db->from('reservation');
		$this->db->join('list','list.id=reservation.list_id');
...
// 
	// $check_paypal = $this->db->where('is_enabled',1)->where('payment_name','Paypal')->get('payments')->num_rows();
		...
		if($this->input->post('payviastripe'))
			{
							
		$check_paypal = $this->db->where('is_enabled',1)->where('payment_name','Stripe')->get('payments')->num_rows();
		...
		if($check_paypal == 0)
		{
			...
			redirect_admin('payment/finance');
					...
					$business         = $this->input->post('wallet_id');
					$currency_code    = $this->input->post('currency');
					$bank_account_id  = $this->input->post('bank_id');
					// Verify return
	...
	$amount = $amount * 100 ;
	$bank_account_id = $this->db->get_where('payout_preferences', array('recipient_id' => $business ))->row()->bank_accountid;
 $error = NULL;
//Send the request
try{
...
$transfer = Stripe_Transfer::create(array(
  ...
  "currency" => "usd",
  "recipient" => $host_payout_id,
  "bank_account" => $bank_account_id,
  "statement_descriptor" => "SALES")
);
...
$this->session->set_userdata('payout_id',$host_payout_id);

if($error!=NULL){
   	
   $this->session->set_flashdata('flash_message', $this->Common_model->flash_message('error',translate("Stripe business account is misconfigured. Please contact your Administrator.")));
   redirect_admin('payment/paypal_cancel');
}
if (isset($transfer->status) && ($transfer->status == "pending" )) {
	
  $this->session->set_userdata('wallet_transid',$transfer->id );	
 $this->session->set_userdata('paid_to', $this->input->post('topaytype') );
 $this->session->set_userdata('paid_to1','common');
 redirect_admin('payment/paypal_success/'.$reservation_id);
}
else
{
	$this->session->set_userdata('error_msg',"Failed transfer from admin wallet to host");
 
 redirect_admin('payment/paypal_cancel');
}
			}
else if($this->input->post('payviastripe_guest')){ 
...
if (isset($transfer->status) && ($transfer->status == "pending" )) {
  ...
  $this->session->set_userdata('paid_to1','guest');
 redirect_admin('payment/paypal_success/'.$reservation_id);
}
else
{
...
else if($this->input->post('payviapaypal'))
			{
...
$host_payout_id = $business;	
if($this->input->post('to_host') && $this->input->post('to_host')!="")
{
...
if($this->input->post('to_guest') && $this->input->post('to_guest')!=""){
		$id = $this->db->where('id',$reservation_id)->get('reservation')->row()->paypal_transactionid;
		$method="RefundTransaction";
		$nvpstr = "&TRANSACTIONID=" . urlencode($id);
		$nvpstr.= "&REFUNDTYPE=" . urlencode("Partial");
		$nvpstr.= "&AMT=" . $amount;
		$get_ec_return = $this->paypal_ec->hash_call($method,$nvpstr);
		
		//echo "<pre>";print_r($get_ec_return);exit;

		if("SUCCESS" == strtoupper($get_ec_return["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($get_ec_return["ACK"]))
		{
...
else if($this->input->post('to_guest') && $this->input->post('to_guest')!=""){
	$id = $this->db->where('id',$reservation_id)->get('reservation')->row()->paypal_transactionid;
		$method="RefundTransaction";
		$nvpstr = "&TRANSACTIONID=" . urlencode($id);
		$nvpstr.= "&REFUNDTYPE=" . urlencode("Partial");
...
if($this->input->post('payviastripe_host'))
{
				$check_paypal = $this->db->where('is_enabled',1)->where('payment_name','Stripe')->get('payments')->num_rows();
...
else if($this->input->post('payviastripe_accept_pay')){
...
if (isset($transfer->status) && ($transfer->status == "pending" )) {
  	...
  	$this->session->set_userdata('wallet_transid',$transfer->id );	
 	$this->session->set_userdata('paid_to','host');
 	$this->session->set_userdata('paid_to1','host');
...
else if($this->input->post('payviabraintree'))
{				
...
{
	//echo $e;
 $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error',translate_admin('PayPal Service is currently Not Available Please Try Again Later.')));
 redirect_admin('payment/finance');
}
if($result->success == 1)
{
...
$result              = $this->Common_model->getTableData( 'reservation',array('id' => $reservation_id) )->row();
		...
		$current_code_value = $result->currency;
		$refund_amt_sms =  	$current_code_value.$topay_amount;
		...
		//}
		////////////////////// send sms to guest /////////
			$checkindate =  get_user_times($checkin, get_user_timezone());
			$checkoutdate =  get_user_times($checkout, get_user_timezone());
			$user_result = $this->Common_model->getTableData('profiles',array('id'=>$result->userby))->row();
			$no_user = $user_result->phnum;
$msg_content = 'Admin refund the amount ('.$refund_amt.') to you for '.get_list_by_id($list_id)->title." ( ".$checkindate." - ".$checkoutdate." )" ;
 			send_sms_user($no_user,$msg_content);	
			////////////////////// send sms to guest /////////
			
			////////////////////// send sms to admin /////////
			$user_result2 = $this->Common_model->getTableData('profiles',array('id'=>1))->row();
			$no_user2 = $user_result2->phnum;
$msg_content2 = 'You have refunded the amount ('.$refund_amt.') to '.ucfirst($traveler_name).' for '.get_list_by_id($list_id)->title." ( ".$checkindate." - ".$checkoutdate." )" ;
...
else if($this->input->post('payviabraintree_accept_pay'))
			{
...
catch(Exception $e)
{
 ...
 $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error',translate_admin('Braintree Service is currently Not Available Please Try Again Later.')));
 redirect_admin('payment/finance');
}
if($result->success == 1)
{
...
Braintree_Transaction::find($refund->refundedTransactionId)->refundId;
		...
		$hotelier_email      = $query1->row()->email;
		   ...
		   $current_code_value = $accept_pay_result->currency ;
		   $refund_amt_sms =  	$current_code_value.$accept_pay_result->amount;
		...
		////////////////////// send sms to host /////////
			$checkindate =  get_user_times($checkin, get_user_timezone());
			$checkoutdate =  get_user_times($checkout, get_user_timezone());
			$user_result = $this->Common_model->getTableData('profiles',array('id'=>$result->userto))->row();
			$no_user = $user_result->phnum;
$msg_content = 'Admin refund the amount ('.$refund_amt.') to you for Host reservation accept commission regarding '.get_list_by_id($result->list_id)->title." ( ".$checkindate." - ".$checkoutdate." )" ;
 			send_sms_user($no_user,$msg_content);	
			////////////////////// send sms to host /////////
			...
			$user_result2 = $this->Common_model->getTableData('profiles',array('id'=>1))->row();
			$no_user2 = $user_result2->phnum;
$msg_content2 = 'You have refunded the amount ('.$refund_amt.') to '.ucfirst($hotelier_name).' for Host reservation accept commission regarding '.get_list_by_id($result->list_id)->title." ( ".$checkindate." - ".$checkoutdate." )" ;
 			send_sms_user($no_user2,$msg_content2);	
			////////////////////// send sms to admin /////////
	...
	{			
		...
		*/
		...
		if($this->session->userdata('paid_to') == 'host'  || $this->session->userdata('paid_to') == 'common') 
		{
			...
			if(isset(get_userPayout($result->userto)->email))
			{
				$host_payout_id =get_userPayout($result->userto)->email;
			}
			else {
				$host_payout_id = '';
                     	...
                     	if($host_payout_id = '')
			{
				$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success',translate_admin('Please fill the payout preference.')));
			...
			$refund_amt_sms =  	$result->currency.':'.$topay_amount; 
			...
			$user_result = $this->Common_model->getTableData('profiles',array('id'=>$result->userto))->row();
			$no_user = $user_result->phnum;
$msg_content = 'Admin refund the amount ('.$refund_amt_sms.') to you for '.get_list_by_id($list_id)->title." ( ".$checkindate." - ".$checkoutdate." )" ;
 			send_sms_user($no_user,$msg_content);	
			////////////////////// send sms to host /////////
			...
			$user_result2 = $this->Common_model->getTableData('profiles',array('id'=>1))->row();
			$no_user2 = $user_result2->phnum;
$msg_content2 = 'You have refunded the amount ('.$refund_amt_sms.') to '.ucfirst($hotelier_name).' for '.get_list_by_id($list_id)->title." ( ".$checkindate." - ".$checkoutdate." )" ;
 			send_sms_user($no_user2,$msg_content2);	
			////////////////////// send sms to admin /////////
			...
			$email_name = 'refund_guest';
			$refund_amt_sms =  	$result->currency.$this->session->userdata('guest_topay'); 
			$splVars = array("{site_name}" => $this->dx_auth->get_site_title(),"{currency}"=>$currency_symbol,"{price}"=>$result->price,"{account}" => "PayPal $host_payout_id","{traveler_name}" => ucfirst($traveler_name), "{host_name}" => ucfirst($hotelier_name), "{list_title}" => get_list_by_id($list_id)->title, "{payed_date}" => date("F j, Y",time()), "{checkin}" => date("F j, Y",$checkin), "{checkout}" => date("F j, Y",$checkout), "{refund_amt}" => $refund_amt);
			//Send Mail
			$this->Email_model->sendMail($traveler_email,$admin_email,ucfirst($admin_name),$email_name,$splVars);
...
function download($param = '')
	{
		if($param !="")
		{
			...
			$pdfFilePath = FCPATH."invoice/Invoice-".$param.".pdf";
			
			if(file_exists($pdfFilePath))
			{
				$data = file_get_contents($pdfFilePath); // Read the file's contents
				$name = "Invoice-".$param.".pdf";
				force_download($name, $data);
	    	}
	    	else// if file not there then create and download
	    	{
				get_invoice_pdf($param);
			...
			{
				$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error',translate_admin('Sorry, Invalide Details!')));
				redirect_admin('payment/finance');
			}
	}

...
public function mobile_verification_settings()
	{
		if($this->input->post('update'))
		{
			$data['string_value']    = $this->input->post('nexmo_api_key');
			$this->db->where('code', 'NEXMO_API_KEY');
			$this->db->update('settings',$data);
			
			$data['string_value']    = $this->input->post('nexmo_api_secret');
			$this->db->where('code', 'NEXMO_API_SECRET');
			$this->db->update('settings',$data);
			
			$data['string_value']    = $this->input->post('nexmo_api_phno');
			$this->db->where('code', 'NEXMO_API_PHONE_NO');
			$this->db->update('settings',$data);
		...
		{
	 $data['nexmo_api_key']    = $this->db->get_where('settings', array('code' => 'NEXMO_API_KEY'))->row()->string_value;
	 $data['nexmo_api_secret']    = $this->db->get_where('settings', array('code' => 'NEXMO_API_SECRET'))->row()->string_value;
	 $data['nexmo_api_phno']    = $this->db->get_where('settings', array('code' => 'NEXMO_API_PHONE_NO'))->row()->string_value;
		
		$data['message_element'] = "administrator/social/mobile_verification_settings";
		$this->load->view('administrator/admin_template', $data);
		}
	}
...
public function sinch_settings()
    {
        if($this->input->post('update'))
        {       
            $data1['string_value']    = $this->input->post('sinch_name');
			$data1['created']    = time();
            $this->db->where('code', 'sinch_name');
            $this->db->update('settings',$data1);
            
            $data2['string_value']    = $this->input->post('sinch_api_key');
            $this->db->where('code', 'sinch_api_key');
            $this->db->update('settings',$data2);
            
            $data3['string_value']    = $this->input->post('sinch_api_secret');
            $this->db->where('code', 'sinch_api_secret');
            $this->db->update('settings',$data3);
        ...
        $data['message_element'] = "administrator/social/sinch_settings";
        $this->load->view('administrator/admin_template', $data);
        }
    }

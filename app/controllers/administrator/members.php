...
// Export CSV
		...
		extract($this->input->get());
				if(isset($time)){
								if($time == "today")
				{
		$data['users'] = $this->Users_model->get_all_today();
				}else{
	$data['users'] = $this->Users_model->get_all()->result();	
				}				
				}else{
		// Get all users
	$data['users'] = $this->Users_model->get_all()->result();	
...
function _check_user_email($email)
	{
		...
		if($this->input->post())
		{
               ...
               'verification_status'   => $this->input->post('check'),
				...
				$this->db->where('id', $id);
				$this->db->update('users',array('timezone' => $this->input->post('timezones')));
	...
	$data['status']=$this->Contacts_model->get_status($id);
		$user_id    = $this->dx_auth->get_user_id();
	        ...
	        $rows = $this->Common_model->getTableData('users', array('id' => $id))->row();
...
// Export CSV
	...
	//	$data['users'] = $this->Users_model->getuserselected($search,$offset, $row_count)->result();
		$data['users'] = $this->db->like('email',$search)->or_like('username',$search)->get('users')->result();
		$data['numrows'] = $this->db->like('email',$search)->or_like('username',$search)->get('users')->num_rows();
		//print_r($search);exit;
		// Pagination config
...
public function passport_verification(){
		$this->load->model('Common_model');
	 			$user_id    = $this->dx_auth->get_user_id();	
				$this->form_validation->set_rules('file1', 'file1', 'trim|xss_clean|pdf');
				$this->form_validation->set_rules('file2', 'file2', 'trim|xss_clean|png');
				...
				if($this->form_validation->run())
	  	        {
                      $config['upload_path'] = './images/';
					   ...
					   if($this->upload->do_upload('file1'))
					   {
						...
						//  redirect('administrator/members/edit/'.$user_id,'refresh');   
					}
						if(!$this->upload->do_upload('file2') && !$this->upload->do_upload('file1'))
					   {
					   	//$error = $this->upload->display_errors();
						$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','You did not select a file to upload.'));
					   	redirect('administrator/members/edit/'.$user_id,'refresh');
					   }
                       if($this->upload->do_upload('file2'))
					   {
						  ...
						  $this->Common_model->passport($data,$user_id);
						   ...
						   $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error',$errorf));
						   
					} 
					     $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','File upload successfully'));
        $data['verify']=$this->Common_model->get_verify($room_id);
		redirect('administrator/members/edit/'.$user_id,'refresh');	
				}
$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','File upload successfully'));
        $data['verify']=$this->Common_model->get_verify($room_id);
...
public function delete(){
	...
	$path=getcwd();	
		...
		unlink($path.'/images/'.$var);
		//echo($result);
		//exit;
	$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','File has been deleted successfully'));
	redirect('administrator/members/edit/'.$user_id,'refresh');	

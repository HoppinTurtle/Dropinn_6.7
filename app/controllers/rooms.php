...
public function Rooms()
	...
	$conditions             = array("id" => $room_id, "list.status" => 1,"list.is_enable" => 1);
	 
	 $result_check                 = $this->Common_model->getTableData('list', $conditions);
	 
	 $conditions1             = array("id" => $room_id);
	 
	 $total_status = 0 ;
	 	 $lys_status                 = $this->Common_model->getTableData('lys_status', $conditions1);
		 $result                 = $this->Common_model->getTableData('list', $conditions1);
	 if($result_check->num_rows() > 0)
		{
			
			if($lys_status->num_rows() > 0)
	 		$total_status = $lys_status->row()->calendar+$lys_status->row()->price+$lys_status->row()->overview+$lys_status->row()->address+$lys_status->row()->photo+$lys_status->row()->listing;	 	
			else
			$total_status = 0;
		}
	 else
	 {
		 if(isset($preview) && ($result->row()->user_id!=$this->dx_auth->get_user_id()))	 
		 {
		 	redirect('info');		
		 }	
	 }
	 
	   if($lys_status->num_rows() == 0 || $total_status < 6 )
	 {
		 if(isset($preview) && ($result->row()->user_id!=$this->dx_auth->get_user_id()))	 
		 {
		 	redirect('info');		
	 ...
	 $data['preview'] = $preview;
		...
		else
		{
		  ...
		  $check_prop = $this->db->where('id' ,$list->property_id)->get('property_type');
		  if($check_prop->num_rows() > 0)
		  {
			$data['property_type']    = $check_prop->row()->type;
		  }
		  else {
			  $data['property_type']    = 'No Property Type';
		  }
	 ...
	 /// similar listing //		
	$ans = $this->db->select('id,lat,long')->get_where('list',array("id !=" => $room_id, "status =" => 1, "is_enable ="=>1));		
	
		  $count = 0;
        $similar_list_ids = array();
		$similar_list_distance = array();
		$similar_list_ids_filter = array();
		...
		$data['count_similar'] = $count;
				  if($count > 0)
				  {

$count_lys = $this->db->select('id,calendar,price,listing,address,overview,photo')->where_in("id", $similar_list_ids)->where(array("calendar !=" => 0,"price !=" => 0,"overview !=" => 0,"address !=" => 0,"photo !=" => 0,"listing !=" => 0))->get('lys_status');

//echo $this->db->last_query();exit;
	if($count_lys->num_rows() > 0)
	{
		 foreach($count_lys->result() as $a ){
		 $similar_list_ids_filter[] = 	$a->id ;				
		 }
	$list_similar_array = $this->db->select('id,title,room_type,user_id,price')->where_in("id", $similar_list_ids_filter)->order_by('id','desc')->limit('6')->get('list');
	$data['similar_listing'] =	$list_similar_array;

	}else{
	$data['count_similar'] = 0;			
	}
	...
	$data['count_similar'] = 0;				  	
		 ...
		 // //// end //
$data['calendarType'] = $list->calendar_type;
	...
	public function lys_new()
	{
		...
		$room_id = $this->db->where('type',$room)->get('room_type');
		...
		if ($room_id->num_rows() != 0)
		{
			$room_id = $room_id->row()->id;
		}
		else {
			
			echo 'Room_type_error';exit;
		}
		...
		$data['country'] = $country;
				...
				$this->session->set_userdata('get_mycountry',$country);
...
//edit function if condition close
	...
	//	print_r($result);
		...
		$data['min_stay']         = $result->min_stay;
		$data['max_stay']         = $result->max_stay;
	...
	{	$data['week_price'] = $price_table->week;
		...
		$data['currency_result'] = $this->db->where('status',1)->get('currency');
	   	$data['list_photo'] = $this->db->where('list_id',$room_id)->order_by('item_id','asc')->get('list_photo');
...
/* output an image tag with our map as the source */
	...
	extract($this->input->post());
	$result = $this->db->where('id',$list_id)->where('is_enable',1)->where('list_pay',1)->where('status',1)->get('list');
...
function update_caltype()
{
	extract($this->input->post());
	$data['calendar_type'] = $cal_type;
	$data1['calendar'] = 1;
    $this->db->where('id',$list_id)->update('list',$data); //For list calendar  type
	$this->db->where('id',$list_id)->update('lys_status',$data1); //For list calendar status
	$this->db->where('list_id',$list_id)->delete('calendar');
...
function getCalType(){
	extract($this->input->post());
	$data['type'] = $this->db->where('id',$room_id)->get('list')->row()->calendar_type;
	if($data['type'] == 3)
	{
		
		$result1 = $this->db->where('list_id',$room_id)->where('availability','Available')->get('calendar');
		
		if($result1->num_rows() != 0)
		{
			$data['start_date']= date('m/d/y',$this->db->select_max('booked_days')->where('list_id',$room_id)->where('availability','Available')->get('calendar')->row()->booked_days);
			$data['end_date']= date('m/d/y',$this->db->select_min('booked_days')->where('list_id',$room_id)->where('availability','Available')->get('calendar')->row()->booked_days);
	...
	public function add_currency()
	{
		...
		$t['to']           =$to;
		$data3['price']                =$price;
		$condata['addguests']          =$agprice;
		$condata['cleaning']           =$cprice;
		$condata['month']              =$mprice;
		$condata['security']           =$sprice;
		$condata['week']               =$wprice;
	...
	//	$data3['price']              =$price;
		...
		$agprice=$this->db->where('id',$room_id)->get('price')->row()->addguests;
		$cprice=$this->db->where('id',$room_id)->get('price')->row()->cleaning;
		$mprice=$this->db->where('id',$room_id)->get('price')->row()->month;
		$sprice=$this->db->where('id',$room_id)->get('price')->row()->security;
		$wprice=$this->db->where('id',$room_id)->get('price')->row()->week;
		//print_r($from1);
		//print_r($price);
		$amount1= round(get_currency_value_lys($from1,$to,$price));
		
		$condata['addguests']= round(get_currency_value_lys($from1,$to,$agprice));
		$condata['cleaning']= round(get_currency_value_lys($from1,$to,$cprice));
		$condata['month']= round(get_currency_value_lys($from1,$to,$mprice));
		$condata['security']= round(get_currency_value_lys($from1,$to,$sprice));
		$condata['week']= round(get_currency_value_lys($from1,$to,$wprice));
		...
		$this->db->where('id',$room_id)->update('price',$condata);
	   ...
	   //$this->db->where('id',$room_id)->update('price',$amount);
		...
		$condata['currency']= $currency;
		$condata['amount']= $amount1;
...
// Discount label 1 end 
	...
	public function get_lys_status()
	{
		...
		$count = $result->calendar+$result->overview+$result->price+$result->photo+$result->address+$result->listing;
...
echo $data['amenities'];exit;
	...
	function add_minstay()
	{
		extract($this->input->post());
		$data['min_stay'] = (int)$min_stay;		
	...
	if($max_stay!=0)
	{
		...
		{
		$this->db->where('id',$room_id)->update('list',$data);
		$min_stay= $this->db->where('id',$room_id)->get('list',$data)->row()->min_stay;
		echo $min_stay;
		exit;
		}
		else 
	...
	function add_maxstay()
	{
		extract($this->input->post());
		$min_stay = (int)$min_stay;
		$data['max_stay'] = (int)$max_stay;
		
		if($min_stay < $max_stay)
		{
		$this->db->where('id',$room_id)->update('list',$data);
		}
		$max_stay= (int) $this->db->where('id',$room_id)->get('list',$data)->row()->max_stay;
		echo $max_stay;
		exit;
...
$list_photo = $this->db->where('list_id',$room_id)->order_by('id','asc')->get('list_photo');
   	...
   	if($list_photo->num_rows() != 0)
	{
     ...
     {    
      ...
      <textarea rows="3" id="highlight_'.$row->id.'" placeholder="'.translate("What are the highlights of this photo?").'" class="input-large" onkeyup="jQuery(this).highlight('.$row->id.')">'.trim($row->highlights).'</textarea>
      </div></li>';
     }
	}
   echo '</div>
...
function add_photo1()
 ...
 foreach ($_FILES["upload_file1"]["error"] as $key => $error) {
                ...
                try{
                  ...
                  if(isset($cloudimage) == '')
                  {
                  		echo "<script>alert('Chosen image cannot be uploaded!');</script>";
                  }
				  else {  
                 ...
                 $ext = pathinfo($name, PATHINFO_EXTENSION);
                    ...
                    if( $secureimage!=''){
                        ...
                        if($image_name != '')
							...
							$temp = $this->db->where('list_id',$this->uri->segment(3))->get('list_photo')->row()->item_id;
							$tmp = $temp+1;
							$insertData['item_id'] = $tmp;
						    ///$this->db->insert('list_photo',$insertData);
...
$list_photo = $this->db->where('list_id',$room_id)->order_by('id','asc')->get('list_photo');
    ...
    foreach($list_photo->result() as $row)
     {    
      $echodata .= '<li class="photo_img_sub" id="'.$row->id.'">
      <div id="pannel_photo_item_id" class="pannel_photo_item">
      <div class="first-photo-ribbon"></div>
      <div class="photo-drag-target"></div><a class="media-link"><img width="100%" src="'.base_url()."images/".$room_id."/".$row->name.'"></a>
      <button data-photo-id="29701026" class="delete-photo-btn js-delete-photo-btn" onclick="jQuery(this).delete_photo('.$row->id.')">
...
function delete_photo()
{
  ...
  "api_secret" => cloud_s_key
	...
	// $image=\Cloudinary\Uploader::destroy("images/50/Screenshot_20161002-023404.png");
	...
	if($this->input->post())
                    {
                         ...
                         try{
                                                              ...
                                                              $image=\Cloudinary\Uploader::destroy("images/".$list_id."/".$name1, array(
...
function final_photo()
{
	...
	$result = $this->db->where('id',$room_id)->get('lys_status')->row();
	$create = $this->db->where('id',$room_id)->get('list')->row();
	...
	if($total == 6)
	{
	  ...
	  $username 		= $query_user->username;
	    ...
	    $title_sms = $this->db->where('id',$room_id)->get('list')->row()->title;
		
		if($edit_list == 'edit')
		{
				$msg_list="$username edited a list"; 
			  $data_message["text"]=$username." edited a list named by ".$title_sms;
		  $insertData = array(
			'list_id'         => $room_id,
		 ...
		 $msg_list="$username created a new list";
		 $data_message["text"]="You created a new list named by ".$title_sms;
		...
		$admin_to_email = $this->Common_model->getTableData('users',array('id' => 1))->row()->email;
			...
			$link = '<a href="'.base_url().'rooms/'.$room_id.'">'.base_url().'rooms/'.$room_id.'</a>';
...
//	$message=array('text' => "".$username." created a new list named by ".$title_sms."");
	...
	$to=$to_phone_number;
	$message=array('text' =>"You edit the list named by ".$title_sms."");
	$response = $this->nexmo->send_message($from, $to, $message);
	//echo $response;exit;
	//echo $this->nexmo->get_http_status();
	}		
...
$through=$to_date_form;
	...
	public function calendar_tab_inner($param = '')
	{
	  if($param == '')
		...
		$data['list_price'] =$this->Common_model->getTableData('price',$condition)->row()->night;
			...
			//////
		$array_book = array();
		foreach($data['result_cal']  as $row)
		{
			$array_book[] = $row->booked_days;
		}
		...
		$pre_schedules  = '{cl: "av", sty: "both", isa: 1},';
			...
			if((!empty($array_book)) && in_array(get_gmt_time(strtotime($full_date)),$array_book))
			{
		$pre_schedules  = '{cl: "av_book", sty: "both", isa: 1},';
			}
		...
		$data['array_book'] = $array_book ;
		///
			
	  $response = $this->load->view(THEME_FOLDER.'/rooms/view_calendar_tab',$data);
	  echo $response;
	}
...
//$list_id = $this->db->where('id',$photo_id)->get('list_photo')->row()->list_id;
	...
	/* if($this->input->post())
                    {
                         ...
                         try{
                                                              ...
                                                              $image=\Cloudinary\Uploader::destroy("images/".$id, array(
                                                                                      //"public_id" => "images/".$room_id));
                                                       //"/".$row->name
                                                       "invalidate" => TRUE,));
													  // echo $name;
 ...
 if( ($this->dx_auth->is_logged_in()) || ($this->facebook_lib->logged_in()) )
        ...
        foreach($check_folder->result() as $row){
        	
			
        		//$name = $this->db->where('id',$list_id)->get('list_photo');
			//$na = $this->db->where('id',$list_id)->get('list_photo')->row()->name;
        //$name = $this->db->where('list_id',$id)->get('list_photo');
		       // $na = $name->row()->name;
	...
	$name1=$pieces[0];
			 try{
                                                              ...
                                                              $image=\Cloudinary\Uploader::destroy("images/".$id."/".$name1, array(
                                                                                      //"public_id" => "images/".$room_id));
                                                       //"/".$row->name
                                                       "invalidate" => TRUE,));
													  // echo $name;
	...
	$to_phone_number = $this->db->where('id',1)->get('profiles')->row()->phnum;
	$to=$to_phone_number;
	$message=array('text' => "".$username." deleted a list named by ".$title." please check the mail for more details.");
	$response = $this->nexmo->send_message($from, $to, $message);
	}
else{
	$from_phone_number = $this->db->get_where('settings', array('code' => 'NEXMO_API_PHONE_NO'))->row()->string_value;
	...
	$to_phone_number = $this->db->where('id',1)->get('profiles')->row()->phnum;
	$to=$to_phone_number;
	$message=array('text' => "".$username." deleted a list named by ".$title.".");
	$response = $this->nexmo->send_message($from, $to, $message);
	...
	$to_phone_number = $this->db->where('id', $id_user)->get('profiles')->row()->phnum;
	$to=$to_phone_number;
	$message=array('text' => "Your list is deleted named by ".$title." please check the mail for more details.");
	$response = $this->nexmo->send_message($from, $to, $message);
	...
	public function ajax_refresh_subtotal()
	{
	  ...
	  $res = $this->Common_model->getTableData( 'list', array('id' => $id ) )->row();
	  $capacity		= $res->capacity;	
		
		$min_stay		= $res->min_stay;
		$max_stay		= $res->max_stay;
		
		$date_diff = floor( strtotime($checkout ) - strtotime( $checkin ) ) / 86400;
		...
		$guests = $guest_count        = $xprice->guests;
		$per_night  = $price    = $xprice->night;
		...
		{
		 	$days = 0;			
   			$data['cal_price'] = $data['price']  = $price;
		...
		for ($i = 1; $i < $total_nights; $i++) {
			...
			for($i=1;$i<$total_nights;$i++)
			{
				...
				$seasonal_arr[] = 	$travel_dates_show[$i]."@price".get_currency_symbol($id).get_currency_value1($id,$seasonal_prices[$travel_dates[$i]]);				
				$count++;
				//// end	
				}else{
				$total_price = $total_price + $xprice->night ;
				/// for showing the list in detail page	
				$seasonal_arr[] = $travel_dates_show[$i]."@price".get_currency_symbol($id).get_currency_value1($id,$xprice->night);
				//// end
				} 
			}
			if(count($seasonal_arr) > 0)
			{
			$seasonal_dates = 	implode(",", $seasonal_arr);	
			}
			if($count < 1)
			{
			$seasonal_dates = "no";	
...
$data['price']    = round($price,0); 		///// Base price ended - base price includes per night, seasonal price prices ///
...
$data['price_after_guest']    = round($price,0);  /// base price + additional guest fee //			 
	...
	$data['sub_total'] = round($price,0);  // subtotal (base price + cleaning fee + security fee)	
...
$data['commission'] = 0;
			 ...
			 if($row->is_premium == 1)
				...
				{
							$fix                = $row->fixed_amount; 
							$data['commission'] = get_currency_value_lys($row -> currency, $res->currency, $fix);
							$amt = $price + get_currency_value_lys($row -> currency, $res->currency, $fix);
							$Fprice             = $amt;
				}
				else
				{  
...
// echo $data['total_price'].' t '.$data['price']. ' p '.$data['commission']. ' c '.$data['sub_total']. ' s ';
// exit;

		//Specific Date changes start here
		$calendarType = $this->db->select('calendar_type')->where('id',$id)->get('list')->row()->calendar_type;
		if($calendarType==3){

			$days1 = getDaysInBetweenC($checkin, $checkout);
			$Status = 0;
		foreach($days1 as $val){
			$availableQuery = $this->db->query("SELECT * FROM `calendar` WHERE `list_id` = '".$id."' AND `availability` = 'Available' AND (`booked_days` = '".get_gmt_time(strtotime($val))."')");
		...
		if($Status!=0){
			echo '{"available":false,"total_price":'.$data['sub_total'].',"reason_message":"Those dates are not available"}';
		...
		$rows  = $query->num_rows();
		$daysexist = $this->db->query("SELECT id,list_id,booked_days FROM `calendar` WHERE `list_id` = '".$id."' AND `availability` != 'Available' AND (`booked_days` >= '".get_gmt_time(strtotime($checkin))."' AND `booked_days` <= '".get_gmt_time(strtotime($checkout))."') GROUP BY `list_id`");
		...
		{
		echo '{"available":false,"total_price":'.$data['sub_total'].',"reason_message":"'.$capacity.' guest(s) only allowed"}';	
		}
		else if ($min_stay!=0 && $min_stay > $date_diff)
		{
		echo '{"available":false,"total_price":'.$data['sub_total'].',"reason_message":"Select minimum '.$min_stay.' day(s)"}';	
		}
		else if ($max_stay!=0 && $max_stay < $date_diff)
		{
		echo '{"available":false,"total_price":'.$data['sub_total'].',"reason_message":"Select maximum '.$max_stay.' day(s)"}';	
		}
		else
		{
		  ...
		  {	
			if($extra_guest == 1)
			  {
			  	 echo '{"seasonal_dates":"'.$seasonal_dates.'","main_total":"'.get_currency_symbol($id).get_currency_value1($id,$data['total_price']).'","service_fee":"'.get_currency_symbol($id).get_currency_value1($id,$data['commission']).'","cal_fee":"'.get_currency_symbol($id).get_currency_value1($id,$data['price']).'","extra_guest_price":"'.get_currency_symbol($id).get_currency_value1($id,$extra_guest_price).'","extra_guest":1,"diff_guest":"'.$data['diif_guest'].'","reason_message":"","price_per_night":"'.get_currency_symbol($id).get_currency_value1($id,$per_night).'","nights":'.$date_diff.',"available":true,"can_instant_book":false,"total_price":"'.get_currency_symbol($id).get_currency_value1($id,$data['sub_total']).'"'.'}';
			  }	
		 }
		else
			{
		
			echo '{"seasonal_dates":"'.$seasonal_dates.'","main_total":"'.get_currency_symbol($id).get_currency_value1($id,$data['total_price']).'","service_fee":"'.get_currency_symbol($id).get_currency_value1($id,$data['commission']).'","cal_fee":"'.get_currency_symbol($id).get_currency_value1($id,$data['price']).'","reason_message":"","price_per_night":"'.get_currency_symbol($id).get_currency_value1($id,$per_night).'","nights":'.$date_diff.',"available":true,"can_instant_book":false,"total_price":"'.get_currency_symbol($id).get_currency_value1($id,$data['sub_total']).'"'.'}';
			}		
	}
...
{ 
   echo "drag"; exit;
    $query=$this->Common_model->getTableData('list', array( 'id' => $this->session->userdata("list_id") ));
    $id=$query->row()->id;
    if(!is_dir( realpath(APPPATH . '../images').'/'.$id))
            {
                mkdir( realpath(APPPATH . '../images').'/'.$id, 0777, true);
            }
      $str = file_get_contents('php://input');
  echo $filename = md5(time().uniqid()).".jpg";
  file_put_contents("images/".$id."/".$filename,$str);
  $insertData=array();
  $insertData['list_id']    =$id;
  ...
  $check = $this->db->where('list_id',$id)->get('list_photo');		

						$photo_status['photo'] = 1;
			            $this->db->where('id',$id)->update('lys_status',$photo_status);
  ...
  $this->watermark($id,$filename);
  $this->watermark1($id,$filename);

  //redirect('rooms/lys_next/edit/'.$id,'refresh');
...
{
	$room_ida = $this->input->post("some");
	 $data['video_code'] =NULL;
										
		$this->db->where('id',$room_ida)->update('list',$data);
...
function youtube_data_grabber()
        {
       	//echo $video_code; 
     //  echo	$this->session->userdata('room_id');exit;
        	$video_code = $this->input->post("video_upload");
			$room_id = $this->input->post("room_id");
...
$data['item_id'] = $order;
$this->db->where('id',$item1)->update('list_photo',$data);
//echo $this->db->last_query();
$order++;	
...
public function get_reserved_date()
    {
				
        extract($this->input->post());
	
           	$all_date = $this->db->select("booked_days")->where("list_id",$roomid)->get('calendar');     
		   	
	        if($all_date->num_rows()>0)
	        {
	        foreach($all_date->result() as $all){
	            $date_get[] = date('d-m-Y',$all->booked_days);                                     
	                 }
	        $unique_block=array_unique($date_get);
			echo implode(",",$unique_block);
...
function spam($param='')
{
		if( (!$this->dx_auth->is_logged_in()) && (!$this->facebook_lib->logged_in()) )
		{
		//$title=preg_replace("/[^a-zA-Z0-9]+/", "_", get_list_by_id($param)->title);	
		$this->session->set_userdata('redirect_to', 'rooms/'.$param.'/');
		redirect('users/signin','refresh');
		}
	        ...
	        $user_id     = $this->dx_auth->get_user_id(); 
			$email       = $mailer_mode = $this->Common_model->getTableData('users', array('id' => $user_id))->row()->email;
			...
			$insert_id           = $this->Common_model->insertData('messages', $data);		
			//$title=preg_replace("/[^a-zA-Z0-9]+/", "_", get_list_by_id($param)->title);				
			$session_lang = $this->session->userdata('locale');
			if($session_lang == "") {
				$email_name = 'spam';	}
			else {$email_name = 'spam_'.$session_lang;	}
			$list_url= site_url().'rooms/'.$param.'/';
			$splvars    = array("{site_name}" => $this->dx_auth->get_site_title(), "{list_url}" => $list_url);
							
			//Send Mail
			$this->Email_model->sendMail($admin_email, $email, $admin_name, $email_name, $splvars);
			
			$this->session->set_flashdata('flash_message', $this->Common_model->flash_message('success','Your request has sent to admin'));

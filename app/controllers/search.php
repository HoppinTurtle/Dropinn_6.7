  ...
  //Get the checkin and chekout dates
   ...
   $this->session->unset_userdata($array_items);
		...
		if($location == '')
		{
			...
			$address_reg     = urlencode($location);
			$address_reg     = str_replace('+','%20',$address_reg); 
			$geocode_reg     = file_get_contents('https://maps.google.com/maps/api/geocode/json?address='.$address_reg.'&sensor=false');
			$output_reg      = json_decode($geocode_reg);
					
			$lat			= $output_reg->results[0]->geometry->location->lat;
		 	$lng			= $output_reg->results[0]->geometry->location->lng;	
		}
...
$data['user_wishlist'] = $this->Common_model->getTableData('user_wishlist',array('user_id'=>$this->dx_auth->get_user_id()));
	   // $data['query']            = '';
		...
		$data['room_type11']      = $room_types1;
		$data['room_type22']      = $room_types2;
		$data['room_type33']      = $room_types3;
    ...
    for ($i=$date_from; $i<=$date_to; $i+=86400) {
      ...
      $ans = $this->db->query("SELECT id,list_id FROM `calendar` WHERE `booked_days` = '".get_gmt_time(strtotime($checkin))."' OR `booked_days` = '".get_gmt_time(strtotime($checkout))."' GROUP BY `list_id`");
		...
		$star_rate      	= $this->input->get('star');
		if(!empty($star_rate))
		{
		  $condition .= " AND (`overall_review` = '".$star_rate."')";
		}
		...
		if(is_array($room_types))
		{
			...
			if(!empty($keywords))
			{
			  $keywords = $this->db->escape_like_str($keywords);
					...
					$condition .= " AND (`address`  LIKE '%".$keywords."%' OR  `title`  LIKE '%".$keywords."%')";
			}
   ...
   //Final query
	 ...
	 // Number of record showing per page
		...
		{
		  $order              = "ORDER BY id ASC, distance";
		} 		
	...
	$condition .= " AND (`id` != '".$row_price->id."')";
		...
		//$data['query']        = $this->db->query("SELECT *, SQRT( POW(69.1 * (list.lat - $lat_reg), 2) + POW(69.1 * (list.long - $long_reg) * COS(list.lat / 57.3), 2)) AS distance FROM (`list`) WHERE $condition $order LIMIT $offset,$per_page");
		
		
		$data['query']        = $this->db->query("SELECT *,(3959 * acos(cos(radians('".$lat."')) * cos(radians(list.lat)) * cos( radians(list.long) - radians('".$lng."')) + sin(radians('".$lat."')) * sin(radians(list.lat)))) AS distance  FROM (`list`) WHERE $condition AND ('distance' < 50)  $order LIMIT $offset,$per_page");
  		
  		$total_rows           =  $this->db->query("SELECT `id` FROM (`list`) WHERE $condition")->num_rows();
...
$overallrateing = $row->overall_review*20;

if($row->lat == 0 && $row->long == 0)
{
	$address_reg     = urlencode($row->street_address);
	$address_reg     = str_replace('+','%20',$address_reg); 
	$geocode_reg     = file_get_contents('https://maps.google.com/maps/api/geocode/json?address='.$address_reg.'&sensor=false');
	$output_reg      = json_decode($geocode_reg);

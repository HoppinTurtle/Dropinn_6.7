...
if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Facebook extends CI_Controller {
		...
		$data2=array("photo_status" => 2,"via_login"=>"facebook","last_login"=>time()) ;
		...
		public function fb_connect($email){
			
		$query=$this->db->get_where('users', array('facebook_email' => $email));
		if($query->num_rows==0){
		...
		if($query->row()->facebook_verify == 'yes' ){
	...
	//	$checknew_user = $this->fb_exists($id);
		...
		else if($this->fb_connect($email)){
			
			$user_info = $this->Users_model->getUserInfobyFbemail_id($email);

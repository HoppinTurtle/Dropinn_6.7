...
function getUserStatusByTwitterUid0($twitter_id) {
	 ...
	 function getUserByTwitterUid($tw_uid)
   {
	...
	function get_all_today()
	{
		...
		$this->db->where("$users_table.created >=", $created);
		$this->db->select("$users_table.*", FALSE);
		$this->db->select("$roles_table.name AS role_name", FALSE);
		$this->db->join('profiles','profiles.id=users.id');
		$this->db->join($roles_table, "$roles_table.id = $users_table.role_id");
...
function get_user_by_id1($user_id)
	...
	function createTwitterUserEmail($twitter_id,$username,$mailId){
...
function getUserInfobyGoogleEmail_id($email){
	
	$this->db->select('id,email,google_id,role_id,username');
     	$this->db->where('google_email',$email);
      	$query = $this->db->get('users');
		$aUserDetails = $query->row_array();
...
function getUserInfobyFbemail_id($email){
...
function getUserInfobyemail_id($email)
	...
	function getUserInfoby_email_id($email)
	{
		$this->db->select('id,email,twitter_id,role_id,username');
     	$this->db->where('email',$email);
      	$query = $this->db->get('users');

...
public function list_post_id()
  ...
  // ID verification start
	  function get_verify($user_id){
	        ...
	        $row=$query->result();
		 foreach($row as $data){
		 	$var1=$data->verification_status; 
		 }
		 return $var1;	 
   }
function view_file1($user_id){
	
			$this->db->where('id',$user_id);
			$query=$this->db->get('users')->row()->file1;
	        return $query;
			
			
}

function view_file2($user_id){
	
			$this->db->where('id',$user_id);
			$query=$this->db->get('users')->row()->file2;
	        return $query;
...
function passport($data,$user_id){
			$this->db->where('id',$user_id);

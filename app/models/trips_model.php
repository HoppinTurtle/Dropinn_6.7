	  ...
	  function Trips_model() 
	  {
			...
			function get_reservation($conditions = array(), $limit = array(), $conditions_or = array())
			{
				...
				if(count($conditions_or) > 0)		
					...
					reservation.coupon_amt,reservation.seasonal_dates_price,reservation.base_price,reservation.guest_count,reservation.host_topay,reservation.guest_topay,reservation.policy,reservation.cancel_date,reservation.is_payed_host,reservation.per_night_price,reservation.is_payed_guest');
...
function get_transaction($conditions = array(), $limit = array(), $conditions_or = array())
			{
				...
				if(count($conditions_or) > 0)		
					...
					reservation.credit_type,reservation.ref_amount,reservation.status,reservation.book_date,
					reservation.is_payed,reservation_status.name,reservation.cleaning,reservation.security,reservation.extra_guest_price,
					reservation.coupon_amt,reservation.seasonal_dates_price,reservation.base_price,reservation.guest_count,reservation.host_topay,reservation.guest_topay');
					
					$result = $this->db->get();
					return $result;			
			}
			function get_invoice($conditions = array(), $limit = array(), $conditions_or = array())
			{
				if(count($conditions) > 0)		
					$this->db->where($conditions);
			...
			$this->db->join('reservation','reservation.id=Invoice.booking_id','inner');
			$this->db->join('users','reservation.userby = users.id');
			$this->db->order_by('Invoice.id','desc');
			$this->db->select('Invoice.created,Invoice.city,Invoice.invoice_number,Invoice.order_number,Invoice.address,Invoice.country,Invoice.company,Invoice.total,reservation.transaction_id,reservation.userby,reservation.userto,Invoice.id,reservation.list_id,Invoice.vat,Invoice.booking_period');
			$result = $this->db->get();
			return $result;			
			}
			...
			function update_invoice($updateKey=array(),$updateData=array())
			{
			  $this->db->update('Invoice',$updateData,$updateKey);
			}

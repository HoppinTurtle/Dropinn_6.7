...
var MapIcons = {
    ...
    init: function() {
        ...
        for (var e = 0; e < 20; e++) {
            MapIcons.numbered[e + 1] = new google.maps.MarkerImage(cdn_img_url + "v1477097492/images/map_icons/map_pins_sprite_001.png", new google.maps.Size(22, 34), new google.maps.Point(0, e * 34));
            MapIcons.numberedHover[e + 1] = new google.maps.MarkerImage(cdn_img_url + "v1477097492/images/map_icons/map_pins_sprite_001.png", new google.maps.Size(22, 34), new google.maps.Point(44, e * 34));
            MapIcons.numberedStarred[e + 1] = new google.maps.MarkerImage(cdn_img_url + "v1477097492/images/map_icons/map_pins_sprite_001.png", new google.maps.Size(22, 34), new google.maps.Point(22, e * 34));
            MapIcons.numberedStarredHover[e + 1] = new google.maps.MarkerImage(cdn_img_url + "v1477097492/images/map_icons/map_pins_sprite_001.png", new google.maps.Size(22, 34), new google.maps.Point(66, e * 34));
            MapIcons.numberedVisited[e + 1] = new google.maps.MarkerImage(cdn_img_url + "v1477097492/images/map_icons/map_pins_sprite_001.png", new google.maps.Size(22, 34), new google.maps.Point(88, e * 34));
            MapIcons.numberedVisitedHover[e + 1] = new google.maps.MarkerImage(cdn_img_url + "v1477097492/images/map_icons/map_pins_sprite_001.png", new google.maps.Size(22, 34), new google.maps.Point(132, e * 34));
            MapIcons.numberedVisitedStarred[e + 1] = new google.maps.MarkerImage(cdn_img_url + "v1477097492/images/map_icons/map_pins_sprite_001.png", new google.maps.Size(22, 34), new google.maps.Point(110, e * 34));
            MapIcons.numberedVisitedStarredHover[e + 1] = new google.maps.MarkerImage(cdn_img_url + "v1477097492/images/map_icons/map_pins_sprite_001.png", new google.maps.Size(22, 34), new google.maps.Point(154, e * 34))
        }
...
var CogzidelSearch = {
    ...
    init: function(e) {
        ...
        if (e.sort) {
         ...
         if (e.star) {
        	$('input[name=review_selector]:checked', '#reviews_stars').val()(e.star)
            CogzidelSearch.params.star = e.star
        }
        if (e.neighborhoods) {
        ...
        $("#keywords").live("keyup", function(e) {
          ...
          $("#keywords").live("focusout", function(e) {
	    ...
	    google.maps.event.addListener(autocomplete, 'place_changed', function() {
	      ...
	      var lng = place.geometry.location.lng();
	       var add = place.formatted_address;
	                  $('#location').val(add);
	                   jQuery("#slocation").val(add);
	                   var lnc=add;
	            jQuery("#currency_mname").replaceWith('<span id="currency_mname" style="float:right">' + lnc + " </span>");
	      ...
	      $('#longi').val(lng);
	      				$("#search_type_list").trigger("click");
	                CogzidelSearch.loadNewResults();        // do your thing
	    });	
	       ...
	       $("#location").live("keypress",function(e)
            ...
            //jQuery('#reviews_stars input').on('change', function() {
            jQuery("#reviews_stars input").change(function() {
			   var star = jQuery('input[name=review_selector]:checked', '#reviews_stars').val(); 
			   $("#search_type_list").trigger("click");
                CogzidelSearch.loadNewResults()
			});
            ...
            jQuery("#map_options label,#redo_img").live("click",function(){
            		
            		//alert($(this).find('label').text())
            			//if($(this).find('label').text()!=srch_map_txt){
                    //$('#redo_search_in_map').attr('checked', false)
                   //}
                  
            		$("#for_redo_search_in_map").text(srch_map_txt);
            		$("#redo_img").css("display","none");
            		$("#redo_search_in_map").css("display","inline-block") ;
            		$("#map_options").css({ });
            		$("#map_options,#map_options label").css({"cursor":"default","font-size" : "12px" ,"color":"white","font-weight" : "normal" });
    ...
    setParamsFromDom: function() {
        ...
        switch (CogzidelSearch.currentViewType) {
         ...
         CogzidelSearch.params.sort = $("#sort").val() || "0";
        CogzidelSearch.params.star = jQuery('input[name=review_selector]:checked', '#reviews_stars').val() || "0";
		...
		CogzidelSearch.params.lat = jQuery("#latti").val();
        CogzidelSearch.params.lng = jQuery("#longi").val();
        ...
        CogzidelSearch.params.guests = guest; }
        else{
        CogzidelSearch.params.guests = jQuery("#number_of_guests").val();}
        
        var sort = sessionStorage.getItem('sort');
        if(sort != null && sort != "") {
        	CogzidelSearch.params.sort = sort; }
        else {
        ...
        if(star != null && star != "") {
        	CogzidelSearch.params.star = star; }
        else {
        	CogzidelSearch.params.star = jQuery('input[name=review_selector]:checked', '#reviews_stars').val(); }
        	
        	
        var min_bedrooms = sessionStorage.getItem('min_bedrooms');
        if(min_bedrooms != null && min_bedrooms != ""){
        	CogzidelSearch.params.min_bedrooms = min_bedrooms; }
        else {
        	CogzidelSearch.params.min_bedrooms = jQuery("#min_bedrooms").val(); }
        	
        var min_bathrooms = sessionStorage.getItem('min_bathrooms');
        if(min_bathrooms != null && min_bathrooms != "") {
        	CogzidelSearch.params.min_bathrooms = min_bathrooms; }
        else {
        	CogzidelSearch.params.min_bathrooms = jQuery("#min_bathrooms").val();}
     
     	var min_beds = sessionStorage.getItem('min_beds');
     	if(min_beds != null && min_beds != "") {
     		CogzidelSearch.params.min_beds = min_beds; 	}
     	else {
     		CogzidelSearch.params.min_beds = jQuery('#min_beds').val(); }   
     		
        var price_min = sessionStorage.getItem('min');
        if(price_min != null && price_min != 0) {
     		CogzidelSearch.params.price_min = price_min; 	}
     	else {
     		CogzidelSearch.params.price_min = jQuery('.price_min_ran').val(); }   
     	
     	var price_max = sessionStorage.getItem('max');
        if(price_max != null && price_max != 0) {
     		CogzidelSearch.params.price_max = price_max; 	}
     	else {
     		CogzidelSearch.params.price_max = jQuery('.price_max_ran').val(); } 
     		
        if(localStorage&&localStorage["data"]){
        var localStoredData=JSON.parse(localStorage["data"]);
        var checkboxes=document.getElementsByName('property_type_id');
        for(var i=0;i<checkboxes.length;i++){
            for(var j=0;j<localStoredData.length;j++){
                if(checkboxes[i].value==localStoredData[j]){
                    checkboxes[i].checked=true;
                     CogzidelSearch.params.property_type_id.push(checkboxes[i].value)
                }
            }
        }
      ...
      if(localStorage&&localStorage["data1"]){
        var localStoredData=JSON.parse(localStorage["data1"]);
        var checkboxe=document.getElementsByName('room_types');
        for(var i=0;i<checkboxe.length;i++){
            for(var j=0;j<localStoredData.length;j++){
                if(checkboxe[i].value==localStoredData[j]){
                    checkboxe[i].checked=true;
                     CogzidelSearch.params.room_types.push(checkboxe[i].value)
                }
            }
        }
        ...
        if(localStorage&&localStorage["instant"]){
        var localStoredData=JSON.parse(localStorage["instant"]);
        var instantcheck=document.getElementsByName('amenities');
        for(var i=0;i<instantcheck.length;i++){
            for(var j=0;j<localStoredData.length;j++){
                if(instantcheck[i].value==localStoredData[j]){
                    instantcheck[i].checked=true;
                     CogzidelSearch.params.hosting_amenities.push(instantcheck[i].value)
                }
            }
        }
        localStorage.removeItem('instant');
    }
     ...
     //   sessionStorage.setItem('min',l);
     //   sessionStorage.setItem('max',c);
        if (f !== c || a !== l) {
    ...
    initMapOnce: function(e) {
        ...
        if (AMM.mapLoaded === false) {
            ...
            lng = $("#longi").val();
                ...
                MapIcons.init();
                AMM.mapLoaded = false
                //For drag into gray area
         var lat_srtt = lat ;
         var lng_strt = lng ;
          google.maps.event.addListener(AMM.map, 'dragstart', function() {
          	  var e = AMM.map.getCenter();
          	  lat_strt = e.lat();
          	   lng_strt = e.lng();
		});
   		google.maps.event.addListener(AMM.map, 'dragend', function(){
           
              var sLat =AMM.map.getBounds().getSouthWest().lat();
              var nLat = AMM.map.getBounds().getNorthEast().lat();
              if (sLat < -85 || nLat > 85) {
                   
                  AMM.map.setOptions({
                        center: new google.maps.LatLng(
                            lat_strt , // set Latitude for center of map here
                            lng_strt) // set Langitude for center of map here
                        });         
                 
               }
               //var ne = bounds.getNorthEast(); // LatLng of the north-east corner
//var sw = bounds.getSouthWest();
            	if($('#redo_search_in_map').is(":checked")==false ){
            		$("#for_redo_search_in_map").text(redo_txt);
            		$("#redo_img").css("display","inline-block");
            		$("#redo_search_in_map").css("display","none");            	
               		$("#map_options,#map_options label").css({"cursor":"pointer" ,"font-weight" : "100", "padding-left": "20px","font-size": "14px","text-align" : "center","color":"white"});
            	}
            	else{
            		...
            		$("#redo_img").css("display","none");
            		$("#redo_search_in_map").css({"display":"inline-block"} );
            		$("#map_options").css({"cursor":"default"});
            		$("#map_options,#map_options label").css({"cursor":"default" ,"color":"white"});
            	}
                    ...
                    google.maps.event.addListener(o, "click", function() {
                     ...
                     var slider_html = '<div class="listing-slideshow"><ul id="slider'+r+'" class="rslides" style="overflow: visible ! important;">';
                      $.each(i.picture_ids,function(key,value)
                      {
                    	...
                    	var src_update = "onerror=this.src='"+cdn_img_url+"images/"+oner+"'";
                    	
                      	slider_html += '<li data="'+r+'"><img height="144" width="216" style="position: absolute; top: 0px; left: 0px; z-index: 2; opacity: 1;height:175px !important" src="'+cdn_img_url+'images/'+value+'" '+src_update+' alt="" data="'+r+'"><div class="panel-overlay-bottom-left panel-overlay-label panel-overlay-listing-label" style=""><span class="h3 price-amount" style="font-size:24px;">'+i.price+'</span> <span class="h6 text-contrast" style="font-size:14px;top: -0.5em;font-weight:bold;">'+i.currency_code+'</span>    </div></li>';

...
jQuery(document).ready(function() {
    ...
    function n() {
        ...
        google.maps.event.addListener(t, "dragend", function(e) {
            ...
            r = t.getPosition().lat();
             ij = t.getPosition().lng();
            mpdrg();
         });
            function mpdrg(){
            jQuery.getJSON("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + r + "," + ij + "&sensor=true&key="+places_API+"&language=en", function(e) {
                if (e.status == "OK") {
                    jQuery(".disable_finish").hide();
                    jQuery(".enable_finish").show();
                    jQuery("#hidden_lat").val(r);
                    jQuery("#hidden_lng").val(ij);
                    address = e.results[0].formatted_address;
                    jQuery("#hidden_address").val(address);
                     jQuery("#lys_street_address").val(address);
                    var t = {};
                    
                     // Changes for address autocmplete        
            var place = e.results[0];
            var componentForm = {
      ...
      $("#str_street_address2").text(e.results[0].formatted_address);
    ...
    document.getElementById(component).disabled = false;
    	...
    	if(addressType=="postal_code" || addressType=="postal_code_prefix" ){
    		...
    		if($("#postal_code").val()==""){
    		$("#postal_code").val($("#postal_code_prefix").val());	
    		}
    	}
    	if(addressType == "locality"){
    		$("#str_city_state_address2").text(val) ;
    	}
    		}
  		}
  		for (var s1 = 0; s1 < e.results.length; s1++) {
         for (var s = 0; s < e.results[s1].address_components.length; s++) {
                	var te = e.results[s1].address_components[s].types[0];
                    var o = e.results[s1].address_components[s].types;
                    
                    if (o == "neighborhood,political") {
                        var neigh = e.results[s1].address_components[s].long_name ;
                        
                        document.getElementById("neighborhood").value = neigh;
                        
                    }
                    if(o == "sublocality_level_1,sublocality,political"){
                        var subp = e.results[s1].address_components[s].long_name ;
                  ...
                  /*  for (var n = 0; n < e.results[0].address_components.length; n++) {
                        ...
                        if (o == "administrative_area_level_1,political") {
                            ...
                            jQuery("#administrative_area_level_1").val(t.state);
                        ...
                        if (o == "postal_code" || o == "postal_code_prefix,postal_code") {
                            ...
                            jQuery("#postal_code").val(t.zipcode);
                    ...
                    jQuery.ajax({
                        ...
                        success: function(e) {
                            jQuery.each(e, function(e, t) {
                                ...
                                jQuery("#postal_code").val(t["zip_code"])
                            })
                        }
                    })
                }
...
jQuery(".update_cal").click(function(){
	var type= jQuery(this).attr('data');
	//alert(type);
	jQuery.ajax({
                url: base_url + "rooms/update_caltype",
                type: 'POST',
                data: {list_id:room_id,cal_type:type},
               ...
               jQuery.ajax({
                    ...
                    success: function(n) {
                        jQuery("#currency_symbol_hidden").val(n);
                      	(n, t);
                      	        //setTimeout(function() {
                      	       	 jQuery("#small_length").text("Your price is too low. The minimum is " + n + t + ".");
                        jQuery("#small_week_length").text("Your price is too low. The minimum is " + n + t * 7 + ".");
        				jQuery("#large_week_length").text("Your price is too long. The maximum is " + n + t * 1400 + ".");
      					jQuery("#large_month_length").text("Your price is too long. The maximum is " + n + t * 6e3 + ".");
        				jQuery("#small_month_length").text("Your price is too low. The minimum is " + n + t * 30 + ".");
                     		
                     e(n, t);
                     setTimeout(function() {
                                 jQuery("#price_container .js-standard-price .center_night .input-addon #currency_symbol").replaceWith('<span id="currency_symbol" class="input-prefix-curency"><b>' + n + "</b></span>");
            jQuery("#advance_price_after1 .input-addon #currency_symbol").replaceWith('<span id="currency_symbol" class="input-prefix-curency"><b>' + n + "</b></span>")
        	 jQuery("#clean_textbox .row .col-4 .input-addon #clean_currency").replaceWith('<span class="input-prefix" id="clean_currency">' + n +"</b></span>");
                jQuery("#additional_textbox .row .col-4 .input-addon #additional_currency").replaceWith('<span class="input-prefix" id="additional_currency">' + n + "</b></span>");
                jQuery("#security_textbox .row .col-4 .input-addon #security_currency").replaceWith('<span class="input-prefix" id="security_currency">' + n + "</b></span>")
      }, 500);
    ...
    jQuery("#night_price").focusout(function() {
     ...
     jQuery("#week_price").keyup(function() { 
    
     a = jQuery("#week_price").val();
     
    b = jQuery("#night_price").val();
   
 if(a < b)
    {
    	 jQuery("#weekpriceval").fadeIn();
    	 ///jQuery("#weekpriceval").fadeOut(10000);
    	
    }
    else{
    	  jQuery("#weekpriceval").fadeOut();
    }
    if(a > b)
    {
    	 jQuery("#weekpriceval").fadeOut();
  ...
  //month  
    jQuery("#currency_drop").change(function() {
      ...
      var t = jQuery("#currency_drop option:selected").text();
            ...
            var wp=parseInt(jQuery("#week_price").val()); 
            var mp=parseInt(jQuery("#month_price").val()); 
            var cp=parseInt(jQuery("#cleaning_price").val());
            var agp=parseInt(jQuery("#extra_guest_price").val()); 
            var sp=parseInt(jQuery("#security_price_textbox").val());   
       ...
       jQuery.ajax({
            ...
            success: function(result) {
              ...
              jQuery("#loading").hide();
               ...
               var a = result['amount'] ;
               var addgp = result['addguests'] ;
               var cleanp = result['cleaning'] ;
               var monthp = result['month'] ;
               var secup = result['security'] ;
               var weekp = result['week'] ;
           
              
              // alert("weekp"+weekp+""+"monthp"+monthp+""+"cleanp"+cleanp+""+"addgp"+addgp+""+"secup"+secup);
              
              
      			 jQuery("#night_price").val(a);
      			 jQuery("#week_price").val(weekp);
      			 jQuery("#month_price").val(monthp);
      			 jQuery("#cleaning_price").val(cleanp);
      			 jQuery("#extra_guest_price").val(addgp);
      			 jQuery("#security_price_textbox").val(secup);
            ...
            //edit price
                ...
                jQuery("#clean_textbox .input-addon #clean_currency").replaceWith('<span class="input-prefix" id="clean_currency">' + e +"</b></span>");
                jQuery("#additional_textbox .input-addon #additional_currency").replaceWith('<span class="input-prefix" id="additional_currency">' + e + "</b></span>");
                jQuery("#security_textbox .input-addon #security_currency").replaceWith('<span class="input-prefix" id="security_currency">' + e + "</b></span>")
...
jQuery("#min_stay").keyup(function() {
     	
var min = parseInt(jQuery.trim(jQuery(this).val()));
var max = parseInt(jQuery.trim(jQuery("#max_stay").val()));

if(min <= 0)
 {
jQuery("#min_stay").val("");
jQuery("#minstay0").show();
jQuery("#minstay").hide();
...
jQuery("#max_stay").keyup(function() {
var max = parseInt(jQuery.trim(jQuery(this).val()));
var min = parseInt(jQuery.trim(jQuery("#min_stay").val()));
if(max <= 0)
 {
 	jQuery("#max_stay").val("");
jQuery("#maxstay0").show();
jQuery("#maxstay").hide();
}
else if(max <= min)
 {
...
jQuery("#maxstay0").hide();
 ...
 jQuery("#min_stay").focusout(function() 
 {
      	var min=parseInt(jQuery.trim(jQuery("#min_stay").val()));
       	var max=parseInt(jQuery.trim(jQuery("#max_stay").val()));
        	...
        	jQuery.ajax({
            url: base_url + "rooms/add_minstay",
            type: "POST",
            data: {
                min_stay: min,
                max_stay: max,
     ...
     /* maximum stay */
      ...
      jQuery("#max_stay").focusout(function() {
       ...
       jQuery("#listing_saving").fadeIn();
        ...
        jQuery.ajax({
            url: base_url + "rooms/add_maxstay",
            type: "POST",
            dataType:"html",
            data: {
                max_stay: max,
   ...
   jQuery("#summary").bind("cut copy paste", function(e) {
                            ...
                            if (e == "1") {
                                ...
                                jQuery.ajax({
                                    ...
                                    success: function(e) {
                                    	...
                                    	}else if(e != "no_image.jpg") {
                                            jQuery("#final_photo").replaceWith('<div  class="modal-seelist-img modal-body-list modal-body-picture-list" id="final_photo" style="background: url(' + cdn_url_images + "images/" + room_id + "/" + e + '); background-repeat:no-repeat; background-size:cover"></div>')
                                        }else {
                                            jQuery("#final_photo").replaceWith('<div  class="modal-seelist-img modal-body-list modal-body-picture-list" id="final_photo" style="background: url(' + cdn_url_images + "images/" + e + '); background-repeat:no-repeat; background-size:cover"></div>')
                                        }
                                    }
                                });
                                jQuery.ajax({
                                    ...
                                    success: function(e) {
                                        if (e == 6) {
		            						...
		            						jQuery(".fixed-top").css("position","static");
    ...
    jQuery("#edit_address").click(function() {
        ...
        jQuery("#address_popup1").show();
          jQuery('#location_found_edit').val("not_found");
        if (jQuery.trim(jQuery("#lys_street_address").val()) != "" && jQuery.trim(jQuery("#locality").val()) != "" && jQuery.trim(jQuery("#postal_code").val()) != "") {
    ...
    jQuery("#lys_street_address").keyup(function() {
        if (jQuery.trim(jQuery(this).val()) != "" && jQuery.trim(jQuery("#locality").val()) != "" && jQuery.trim(jQuery("#postal_code").val()) != "") {
    ...
    jQuery(".enable-btn").click(function() {
    	 ...
    	 var cnt_pall = 5;
                    jQuery("#localized-fields input").not("#postal_code_prefix,#neighborhood,#district,#route,#administrative_area_level_1").each(function(){
                    	if(jQuery(this).val() == ""){
                    		cnt_pall--;
                    	}
                    })
            ...
            data: {
                ...
                city: jQuery("#locality").val(),
                state: jQuery("#administrative_area_level_1").val(),
                street_address: jQuery("#lys_street_address").val(),
                optional_address: jQuery("#street_number").val(),
                zipcode: jQuery("#postal_code").val(),
                lat: jQuery("#hidden_lat").val(),
                lng: jQuery("#hidden_lng").val(),
                full_address: jQuery("#hidden_address").val(),
        ...
        jQuery("#str_street_address").replaceWith('<strong id="str_street_address">' + jQuery("#lys_street_address").val() + "</strong>");
                jQuery("#str_city_state_address").replaceWith('<strong id="str_city_state_address">' + jQuery("#locality").val() + "</strong>");
               
                jQuery("#str_zipcode").replaceWith('<strong id="str_zipcode">' + jQuery("#postal_code").val() + "</strong>");
                jQuery("#str_street_address1").replaceWith('<strong id="str_street_address1">' + jQuery("#lys_street_address").val() + "</strong>");
                jQuery("#str_city_state_address1").replaceWith('<strong id="str_city_state_address1">' + jQuery("#locality").val() + "</strong>");
              
                jQuery("#str_zipcode1").replaceWith('<strong id="str_zipcode1">' + jQuery("#postal_code").val() + "</strong>");
                jQuery("#str_street_address2").replaceWith('<strong id="str_street_address2">' + jQuery("#lys_street_address").val() + "</strong>");
                jQuery("#str_city_state_address2").replaceWith('<strong id="str_city_state_address2">' + jQuery("#locality").val() + "</strong>");
               
                jQuery("#str_zipcode2").replaceWith('<strong id="str_zipcode2">' + jQuery("#postal_code").val() + "</strong>")
       ...
       jQuery("#address_popup1").hide();
        ...
        jQuery(".disable_finish").hide();
                   ...
                   // jQuery(".emailadd").text(verify_msg);
                   // jQuery(".addrs_small").text(loc_msg);
            //jQuery("#lochead").text("Location not found");
                    	  //Query("#locsub").text("Manually pin this listing's location on a map.");
                    	  //jQuery("#locpane").text("We couldn't automatically find this listing's location, but if the address below is correct you can manually pin it's location on the map instead.");
    ...
    jQuery(".disable-btn").click(function() {
         Translate_text_alert("Please enter the data")
    });
        ...
        jQuery.ajax({
            ...
            success: function(t) {
                ...
                if (e == "") {
                    ...
                    jQuery("#detail_plus1").hide();
                    jQuery("#detail_plus").hide();
                    jQuery("#detail_plus_tick").show();
                    jQuery("#detail_plus_grn_white").show();
                }
            }
        })
    });
   jQuery("#house_rules_textbox").mouseover(function() {
    ...
    jQuery("#cancel_policy").change(function() {
        jQuery.ajax({
            ...
            success: function(e) {
                ...
                jQuery("#terms_plus_tick").show();
                jQuery("#terms_plus_after1").show();
               }
               else
               {
    ...
    google.maps.event.addListener(A, "place_changed", function() { 
        ...
        var n =e.geometry.location.lng();
          var addr = e.formatted_address;
        jQuery.getJSON("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + t + "," + n + "&sensor=true&key="+places_API+"&language=en", function(e) {
        
            if (e.status == "OK") {
                jQuery("#hidden_lat").val(t);
                jQuery("#hidden_lng").val(n);
                address = e.results[0].formatted_address;
                jQuery("#hidden_address").val(addr);
                var r = 0;
                var i = {};
    ...
    for (var s = 0; s < e.results[s1].address_components.length; s++) {
             ...
             /*   for (var s = 0; s < e.results[0].address_components.length; s++) {
                    ...
                    if (o == "administrative_area_level_1,political") {
                        ...
                        jQuery("#administrative_area_level_1").val(i.state);
                    ...
                    if (o == "postal_code" || o == "postal_code_prefix,postal_code") {
                        ...
                        jQuery("#postal_code").val(i.zipcode);
...
//For Calendar section 3 options
jQuery(document).ready(function(){
	jQuery.ajax({
		url: base_url+'rooms/getListStatus',
		type: 'POST',
		data:{
			list_id: room_id
		},

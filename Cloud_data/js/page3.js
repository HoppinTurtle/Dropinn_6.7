...
function refresh_subtotal() {
    var e = function(e, t, n) {
        ...
        if (e.available) {
            ...
            jQuery("#total_nights").html(e.nights+" nights");
             jQuery("#to_nights").html(e.nights+" nights");
		...
		var html_n ="";
            ...
            jQuery("#guest_price").html(e.guest_price);
            jQuery("#diif_guest").html(e.diff_guest+" guests");
           	jQuery("#mul_price").html(e.cal_fee);
            jQuery("#extra_guest_price").html(e.extra_guest_price);           
            jQuery("#subtotal").html(e.total_price);
            jQuery("#total").html(e.main_total);

            if(e.seasonal_dates != "no" && e.seasonal_dates != "")
			{
				var seasonal = e.seasonal_dates ;
				var parsed = seasonal.split(",");
				html_s +="<tr><td>Special price applied</td></tr>";
			    for(var count=0;count<parsed.length;count++)
			    {
					var stringsplt = parsed[count] ;
					var detail = stringsplt.split("@price");
					html_s +="<tr><td id='td'>On "+detail[0]+"</td>";
					html_s +="<td id='td2'><span class='value' id='seasonal_fee'>"+detail[1]+"</span><br /></td></tr>";
					jQuery("#base_price").html(html_s);
			  	}
			}else{
					html_n +="<tr><td id='td' style='border-top:none;' ><label  id='price_amount'>"+e.price_per_night+"</label> x <span id='total_nights'>"+e.nights+" nights</span>";
					html_n +="<img src='"+base_url+"Cloud_data/images/question_mark_2.png' height='15' class='anchor' /> <p class='message'>Average nightly rate is rounded.</p> </td>";
					html_n += "<td id='td2' style='border-top:none;' > <span id='mul_price'>"+e.cal_fee+"</span> </td></tr></table>";
					jQuery("#base_price").html(html_n);	
			}
    ...
    if (calendar_is_not_set_date("checkin") || calendar_is_not_set_date("checkout")) {
        ...
        jQuery("#subtotal, #total, #book_it_disabled_message").html('<img src="' + base_url + 'Cloud_data/images/spinner.gif" alt="" height="16" width="16" />')
    }
...
var CogzidelRooms = {
    init: function(e) {
        ...
        jQuery("#reputation .pagination a").live("click", function() {
            var e = jQuery(this);
            e.parent().append('<img src="' + base_url + 'Cloud_data/images/spinner.gif" class="spinner" height="16" width="16" alt="" />');
            jQuery.ajax({
                url: e.attr("href"),
                success: function(t) {
